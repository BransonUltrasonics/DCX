/*
$Header:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/FieldbusTable.cpp_v   1.5   17 Nov 2015 07:39:14   sesharma  $
*/
/****************************************************************************/
/*                                                                          */
/*                                                                          */
...
/*************************                         **************************/
/*****************************************************************************
$Log:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/FieldbusTable.cpp_v  $
 * 
 *    Rev 1.5   17 Nov 2015 07:39:14   sesharma
 * ParamAccessPreCallback() modified to fix the issues DEF01208 & DEF01209 i.e. "Software allows to write above the max value" for all energy related parameters.
 * 
 *    Rev 1.4   27 Aug 2015 11:06:46   RJamloki
 * Fixes for parameter types, expected error codes and min,max default read values.
 * 
 *    Rev 1.3   24 Aug 2015 02:12:20   sesharma
 * Fixed access rights for Parameter Id 170.
 * 
 *    Rev 1.2   21 Aug 2015 05:49:56   RJamloki
 * Made memory clear options writable only with token, Fix for issue where read only parameters were getting modified from fieldbus
 * 
 *    Rev 1.1   10 Aug 2015 09:40:04   RJamloki
 * Added clear memory with reset , clear memory before seek, clear with horn scan and  clear memory at power up options in FieldBus Table
 * 
...
ParamAccessError CheckParameterRange(SINT32 ParamIndex , void * Value);
SINT32 GetParameterIndexInTable(UINT16 ParamNumber);
BOOLEAN BOOLEANMin = OFF;
BOOLEAN BOOLEANMax = ON;
...
//Test values read/write default,min and max all in range of signed int
COMMTESTWR TestValuesWR[3]={
							{-100, 200, -30000, 60000u, -2000000000, 2147400000u},
							{-120, 50, -32000, 5000, -2100000000, 200000000},
							{120, 250, 32000, 65000u, 2100000000, 2147483600u}
						   };
...
static UINT32 SysConfigBaseTableIndex;
...
{
		{PARAMINFO(1, TYPE_S8, TYPE_S8), &TestValuesRD.TestValue1Read, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(2, TYPE_8, TYPE_8), &TestValuesRD.TestValue2Read, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(3, TYPE_S16, TYPE_S16), &TestValuesRD.TestValue3Read, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(4, TYPE_16, TYPE_16), &TestValuesRD.TestValue4Read, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(5, TYPE_S32, TYPE_S32), &TestValuesRD.TestValue5Read, 0 , 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(6, TYPE_32, TYPE_32), &TestValuesRD.TestValue6Read,0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(7, TYPE_S8, TYPE_S8), &TestValuesWR[0].TestValue1Write, &TestValuesWR[1].TestValue1Write,&TestValuesWR[2].TestValue1Write, 0, PARAMINFOEXT(ARRAYNA, 0, (RS|WS))},
		...
		{PARAMINFO(9, TYPE_S16, TYPE_S16), &TestValuesWR[0].TestValue3Write, &TestValuesWR[1].TestValue3Write, &TestValuesWR[2].TestValue3Write, 0, PARAMINFOEXT(ARRAYNA, 0, (RS|WS))},
		...
		{PARAMINFO(11, TYPE_S32, TYPE_S32), &TestValuesWR[0].TestValue5Write, &TestValuesWR[1].TestValue5Write, &TestValuesWR[2].TestValue5Write, 0, PARAMINFOEXT(ARRAYNA, 0, (RS|WS))},
		...
		// System Information
		{PARAMINFO(150, TYPE_S32, TYPE_16), &MacchipData.Freq, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		...
		// RealTime Clock
		{PARAMINFO(170, TYPE_32, TYPE_32), &RTDate.RTCDate, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, (WSAT | RS | RPRCB | WPSCB))},
		...
		{PARAMINFO(400, TYPE_S16, TYPE_S16), &ExternalControl.AmplitudeSet, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		//dummy
		...
		{PARAMINFO(420, TYPE_S16, TYPE_S16), &ExternalControl.Amplitude, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(421, TYPE_S16, TYPE_S16), &ExternalControl.Current,0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(422, TYPE_S16, TYPE_S16), &ExternalControl.Power, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(423, TYPE_S16, TYPE_S16), &ExternalControl.Phase, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		//dummy
		{PARAMINFO(424, TYPE_VOID, TYPE_VOID), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(425, TYPE_VOID, TYPE_VOID), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},

		{PARAMINFO(426, TYPE_S16, TYPE_S16), &ExternalControl.PSV, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(427, TYPE_16, TYPE_16), &ExternalControl.Frequency, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		{PARAMINFO(428, TYPE_S16, TYPE_S16), &ExternalControl.Temperature, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, RS)},
		//dummy
		...
		{PARAMINFO(439, TYPE_S16, TYPE_S16), &ExternalControl.ExtFrequencyOffset, 0, 0, 0, PARAMINFOEXT(ARRAYNA, 0, (RS | WSAT | WPRCB))},
		//TODO:Min max
		{PARAMINFO(440, TYPE_S16, TYPE_S16), &ExternalControl.ExtAmplitudeSet, 0,0, 0, PARAMINFOEXT(ARRAYNA, 0, (RS | WSAT | WPRCB))},
		...
		{PARAMINFO(1041, TYPE_S32, TYPE_A_8), &ParameterSet[CurrPreset].WcParam.WeldMode,
		...
		{PARAMINFO(1060, TYPE_32, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.WeldMode,
		&FbMinPs.WcParam.WeldMode, &FbMaxPs.WcParam.WeldMode,
		&ParameterSet[Preset_Default].WcParam.WeldMode, PARAMINFOEXT(ARRAYPS, SAVECRC_WC, ACCPARASET)},
		{PARAMINFO(1061, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.WeldTimeCP,
		...
		{PARAMINFO(1062, TYPE_32, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.EnergyCP,
		...
		{PARAMINFO(1063, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.PeakPowerCP,
		...
		{PARAMINFO(1064, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.ScrubTime,
		...
		{PARAMINFO(1065, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.Amplitude1,
		...
		{PARAMINFO(1066, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.Amplitude2,
		...
		{PARAMINFO(1067, TYPE_S32, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.AmpStepCondition,
		...
		{PARAMINFO(1068, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.AmpStepTime,
		...
		{PARAMINFO(1069, TYPE_32, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.AmpStepEnergy,
		...
		{PARAMINFO(1070, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.AmpStepPower,
		...
		{PARAMINFO(1071, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].DcpParam.WeldRampTime,
		...
		{PARAMINFO(0, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].DcpParam.WeldProfileRampTime,
		...
		{PARAMINFO(1073, TYPE_8, TYPE_A_S32), &ParameterSet[CurrPreset].DcpParam.MemoryStoreFlag,
		...
		{PARAMINFO(1074, TYPE_S32, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.FreqOffset,
		...
		{PARAMINFO(1075, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.HoldTime,
		...
		{PARAMINFO(1076, TYPE_8, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.EBFlag,
		...
		{PARAMINFO(1077, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.EBAmp,
		...
		{PARAMINFO(1078, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.EBTime,
		...
		{PARAMINFO(1079, TYPE_8, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.ABFlag,
		...
		{PARAMINFO(1080, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.ABAmp,
		...
		{PARAMINFO(1081, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.ABTime,
		...
		{PARAMINFO(1082, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.ABDelay,
		...
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},//dummy 1083
		{PARAMINFO(1084, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.ScrubAmp,
		...
		{PARAMINFO(1086, TYPE_32, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.TimeCutOff,
		...
		{PARAMINFO(1087, TYPE_32, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.EnergyCutoff,
		...
		{PARAMINFO(1088, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.PeakPowerCutoff,
		...
		{PARAMINFO(1089, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].LimitsParam.WeldTimeMLR,
		...
		{PARAMINFO(1090, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].LimitsParam.WeldTimePLR,
		...
		{PARAMINFO(1091, TYPE_32, TYPE_A_S32), &ParameterSet[CurrPreset].LimitsParam.EnergyMLR,
		...
		{PARAMINFO(1092, TYPE_32, TYPE_A_S32), &ParameterSet[CurrPreset].LimitsParam.EnergyPLR,
		...
		{PARAMINFO(1093, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].LimitsParam.PowerMLR,
		...
		{PARAMINFO(1094, TYPE_16, TYPE_A_S32), &ParameterSet[CurrPreset].LimitsParam.PowerPLR,
		...
		{PARAMINFO(1095, TYPE_A_S32, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.WeldFreqLowCutoff,
		...
		{PARAMINFO(1096, TYPE_A_S32, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.WeldFreqHighCutoff,
		...
		{PARAMINFO(0, TYPE_A_8, TYPE_A_S32), &ParameterSet[CurrPreset].WcParam.GrdDetectFlagCP,
		...
		{PARAMINFO(1098, TYPE_S32, TYPE_A_S32), &ParameterSet[CurrPreset].ActParam.CustomAIn1Cutoff,
		...
		{PARAMINFO(1099, TYPE_S32, TYPE_A_S32), &ParameterSet[CurrPreset].ActParam.CustomAIn2Cutoff,
		...
		{PARAMINFO(1210, TYPE_A_8, TYPE_A_S8), &WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[0],
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1211, TYPE_A_8, TYPE_A_S8), (UINT8*)&WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[0] + 1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1212, TYPE_A_8, TYPE_A_S8), &WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[1],
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1213, TYPE_A_8, TYPE_A_S8), (UINT8*)&WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[1] + 1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1214, TYPE_A_8, TYPE_A_S8), &WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[2],
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1215, TYPE_A_8, TYPE_A_S8), (UINT8*)&WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[2] + 1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1216, TYPE_A_8, TYPE_A_S8), &WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[3],
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1217, TYPE_A_8, TYPE_A_S8), (UINT8*)&WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[3] + 1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1218, TYPE_A_8, TYPE_A_S8), &WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[4],
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1219, TYPE_A_8, TYPE_A_S8), (UINT8*)&WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[4] + 1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1220, TYPE_A_8, TYPE_A_S8), &WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[5],
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1221, TYPE_A_8, TYPE_A_S8), (UINT8*)&WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[5] + 1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1222, TYPE_A_8, TYPE_A_S8), &WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[6],
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1223, TYPE_A_8, TYPE_A_S8), (UINT8*)&WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[6] + 1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1224, TYPE_A_8, TYPE_A_S8), &WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[7],
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1225, TYPE_A_8, TYPE_A_S8), (UINT8*)&WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[7] + 1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1226, TYPE_A_8, TYPE_A_S8), &WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[8],
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1227, TYPE_A_8, TYPE_A_S8), (UINT8*)&WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[8] + 1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1228, TYPE_A_8, TYPE_A_S8), &WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[9],
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1229, TYPE_A_8, TYPE_A_S8), (UINT8*)&WeldStatusSet[CurrPreset].DcxPs.InfoParam.PresetID[9] + 1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		//dummy
		...
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		//dummy
		{PARAMINFO(1242, TYPE_VOID, TYPE_VOID), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		...
		{PARAMINFO(1260, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.WeldMode,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1261, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.WeldTimeCP,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1262, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.EnergyCP,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1263, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.PeakPowerCP,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1264, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.ScrubTime,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1265, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.Amplitude1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1266, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.Amplitude2,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1267, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.AmpStepCondition,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1268, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.AmpStepTime,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1269, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.AmpStepEnergy,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1270, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.AmpStepPower,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1271, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.DcpParam.WeldRampTime,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(0, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.DcpParam.WeldProfileRampTime,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1273, TYPE_8, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.DcpParam.MemoryStoreFlag,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1274, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.FreqOffset,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1275, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.HoldTime,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1276, TYPE_8, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.EBFlag,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1277, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.EBAmp,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1278, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.EBTime,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1279, TYPE_8, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.ABFlag,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1280, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.ABAmp,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1281, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.ABTime,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1282, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.ABDelay,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1283, TYPE_8, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.ScrubTimeFlag,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1284, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.ScrubAmp,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1285, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.ScrubTime,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1286, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.TimeCutOff,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1287, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.EnergyCutoff,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1288, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.PeakPowerCutoff,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1289, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.LimitsParam.WeldTimeMLR,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1290, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.LimitsParam.WeldTimePLR,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1291, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.LimitsParam.EnergyMLR,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1292, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.LimitsParam.EnergyPLR,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1293, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.LimitsParam.PowerMLR,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1294, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.LimitsParam.PowerPLR,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1295, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.WeldFreqLowCutoff,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1296, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.WeldFreqHighCutoff,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1297, TYPE_8, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.WcParam.GrdDetectFlagCP,
		0, 0, 0, PARAMINFOEXT(0, 0, RS)},//TODO:Ground detect error??
		{PARAMINFO(1298, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.ActParam.CustomAIn1Cutoff, 0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1299, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].DcxPs.ActParam.CustomAIn2Cutoff, 0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		...
		//status values
		{PARAMINFO(1360, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.WeldTime,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1361, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.HoldTime,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1362, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.WeldEnergy,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1363, TYPE_8, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.PeakPower,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1364, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.AvgPower,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1365, TYPE_8, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.AvgAmplitude1,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1366, TYPE_8, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.AvgAmplitude2,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1367, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.RecalledResonanceFrequencyAbs,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1368, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.WeldStartFrequency,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1369, TYPE_16, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.WeldStopFrequency,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1370, TYPE_32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.StoredFrequencyAbs,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1371, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.ResFreqOK,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1372, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.EndAmplitudeSet,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1373, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.EndAmplitude,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1374, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.ErrorPsv,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1375, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.EndPower,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1376, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.EndCurrent,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1377, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.EndPhase,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1378, TYPE_S32, TYPE_A_S32), &WeldStatusSet[CurrPreset].Wstat.EndTemperature,
		0, 0, 0, PARAMINFOEXT(ARRAYSS, 0, RS)},
		{PARAMINFO(1379, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1380, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1381, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1382, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1383, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1384, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1385, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1386, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1387, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1388, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1389, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		//End Weld Status Parameters

		//Stack parameters
		//dummy
		//******** Stack Parameter - Function 0(Seek) *******/
		{PARAMINFO(1460, TYPE_16, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaSeek.SeekTime,
		...
		{PARAMINFO(1461, TYPE_16, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaSeek.SeekAmp,
		...
		{PARAMINFO(1462, TYPE_16, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaSeek.SeekRampTime,
		...
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaSeek.SeekAmpLoopC1,
		...
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaSeek.SeekAmpLoopC2,
		&FbMinHsPs.ParaSeek.SeekAmpLoopC2, &FbMaxHsPs.ParaSeek.SeekAmpLoopC2,
		&HsParameterSet[HsPreset_Default].ParaSeek.SeekAmpLoopC2, PARAMINFOEXT(ARRAYHSPS, SAVECRC_HSPS, ACCPARASET)},
		{PARAMINFO(1465, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaSeek.FreqOffset,
		...
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaSeek.SeekPhaseLoopI,
		...
		{PARAMINFO(1467, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaSeek.SeekFreqLow,
		...
		{PARAMINFO(1468, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaSeek.SeekFreqHigh,
		...
		{PARAMINFO(1469, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1470, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1471, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1472, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1473, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1474, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		//********Stack Parameter - Function 1(Test)*******/
		{PARAMINFO(1475, TYPE_16, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.Amplitude1,
		...
		{PARAMINFO(1476, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.AmpStepCondition,
		...
		{PARAMINFO(1477, TYPE_32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.AmplitudeProfileTime,
		...
		{PARAMINFO(1478, TYPE_16, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.Amplitude2,
		...
		{PARAMINFO(1479, TYPE_16, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.AmplitudeStartRampTime,
		...
		{PARAMINFO(0, TYPE_16, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.AmplitudeProfileRampTime,
		...
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.AmplitudeLoopC1,
		...
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.AmplitudeLoopC2,
		...
		{PARAMINFO(0, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.FreqOffset,
		...
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.PhaseLoopI,
		...
		{PARAMINFO(1485, TYPE_32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.TimeErrorHigh,
		...
		{PARAMINFO(1486, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.WeldFreqLow,
		...
		{PARAMINFO(1487, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaTest.WeldFreqHigh,
		...
		{PARAMINFO(1488, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1489, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		//********Stack Parameter - Function 2(Scan)*******/

		{PARAMINFO(1490, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaScan.FrequencyStart,
		...
		{PARAMINFO(1491, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaScan.FrequencyStop,
		...
		{PARAMINFO(0, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaScan.FrequencyStep,
		...
		{PARAMINFO(1493, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaScan.TimeDelay,
		...
		{PARAMINFO(1494, TYPE_16, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaScan.MaxAmplitude,
		...
		{PARAMINFO(1495, TYPE_16, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaScan.MaxCurrent,
		...
		{PARAMINFO(1496, TYPE_32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].ParaScan.TimeErrorHigh,
		...
		{PARAMINFO(1497, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1498, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1499, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1500, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1501, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1502, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1503, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		{PARAMINFO(1504, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		//Common Stack parameters
		{PARAMINFO(1505, TYPE_S32, TYPE_A_S32), &HsParameterSet[CurrHsPreset].DigitalTuneScaledUP,
		...
		{PARAMINFO(1610, TYPE_16, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsPreset.ParaSeek.SeekTime,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1611, TYPE_16, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsPreset.ParaSeek.SeekAmp,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1612, TYPE_16, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsPreset.ParaSeek.SeekRampTime,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsPreset.ParaSeek.SeekAmpLoopC1,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsPreset.ParaSeek.SeekAmpLoopC2,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1615, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsPreset.ParaSeek.FreqOffset,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsPreset.ParaSeek.SeekPhaseLoopI,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1617, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsPreset.ParaSeek.SeekFreqLow,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1618, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsPreset.ParaSeek.SeekFreqHigh,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		...
		{PARAMINFO(1680, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.Time,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1681, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.AvgAmplitude,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1682, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.RecalledMidBandFreqAbs,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1683, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.SeekStartFrequencyAbs,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1684, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.SeekStopFrequencyAbs,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1685, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.SeekMemoryFrequencyAbs,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1686, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.ResFreqOK,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1687, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.EndSetAmplitude,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1688, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.EndAmplitude,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1689, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.EndPwm,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1690, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.EndPower,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1691, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.EndCurrent,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1692, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.EndPhase,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1693, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][0].HsStatus.SeekStatus.EndTemperature,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		...
		{PARAMINFO(1710, TYPE_16, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.Amplitude1,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1711, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.AmpStepCondition,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1712, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.AmplitudeProfileTime,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1713, TYPE_16, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.Amplitude2,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1714, TYPE_16, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.AmplitudeStartRampTime,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(0, TYPE_16, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.AmplitudeProfileRampTime,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.AmplitudeLoopC1,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.AmplitudeLoopC2,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(0, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.FreqOffset,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		...
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1721, TYPE_S32, TYPE_A_32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.WeldFreqLow,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1722, TYPE_S32, TYPE_A_32), &HsStatusSet[CurrHsPreset][1].HsPreset.ParaTest.WeldFreqHigh,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		...
		{PARAMINFO(1780, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.Time,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1781, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.AvgAmplitude1,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1782, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.AvgAmplitude2,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1783, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.RecalledResFreqAbs,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1784, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.ResFreqOK,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1785, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.TestStartFrequencyAbs,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1786, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.TestStopFrequencyAbs,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1787, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.EndAmplitudeSet,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1788, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.EndAmplitude,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1789, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.EndPwm,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1790, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.EndPower,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1791, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.EndCurrent,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1792, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.EndPhase,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1793, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][1].HsStatus.TestStatus.EndTemperature,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1794, TYPE_VOID, TYPE_A_S32), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		//********End Stack Status Parameter - Function 1(Test)*****/
		//dummy
		{PARAMINFO(1795, TYPE_VOID, TYPE_VOID), 0, 0, 0, 0, PARAMINFOEXT(0, 0, 0)},
		...
		{PARAMINFO(1810, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsPreset.ParaScan.FrequencyStart,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1811, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsPreset.ParaScan.FrequencyStop,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1812, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsPreset.ParaScan.FrequencyStep,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1813, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsPreset.ParaScan.TimeDelay,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1814, TYPE_16, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsPreset.ParaScan.MaxAmplitude,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1815, TYPE_16, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsPreset.ParaScan.MaxCurrent,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1816, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsPreset.ParaScan.TimeErrorHigh,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		...
		{PARAMINFO(1880, TYPE_32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsStatus.ScanStatus.Time,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1881, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsStatus.ScanStatus.ScanStartFrequency,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1882, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsStatus.ScanStatus.ScanStopFrequency,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1883, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsStatus.ScanStatus.EndAmplitude,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1884, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsStatus.ScanStatus.EndPwm,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1885, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsStatus.ScanStatus.EndPower,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1886, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsStatus.ScanStatus.EndCurrent,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1887, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsStatus.ScanStatus.EndPhase,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
		{PARAMINFO(1888, TYPE_S32, TYPE_A_S32), &HsStatusSet[CurrHsPreset][2].HsStatus.ScanStatus.EndTemperature,
		0, 0, 0, PARAMINFOEXT(ARRAYHSSS, 0, RS)},
...
//defines to extract the 8 bit value from the pointer pointing to 8 bit variable
#define VAL8(X)   (*((UINT8*)X))
#define VALS8(X)  (*((SINT8*)X))
//defines to extract the 16 bit value from the pointer pointing to 16 bit variable
#define VAL16(X)  (*((UINT16*)X))
#define VALS16(X) (*((SINT16*)X))
//defines to extract the 32 bit value from the pointer pointing to 32 bit variable
#define VAL32(X)  (*((UINT32*)X))
#define VALS32(X) (*((SINT32*)X))
//defines to extract the lower 8 and 16 bit value from the pointer pointing to 32 bit variable in a Big Endian system.
#define VAL8BE(X)  (*((UINT8*)X + 3))
#define VALS8BE(X)  (*((SINT8*)X + 3))
#define VAL16BE(X)  (*((UINT16*)X + 1))
#define VALS16BE(X)  (*((SINT16*)X + 1))
 ...
 *
 *   Purpose:
 *    To get the parameter index in fieldbus table based on passed parameter ID
 *   Entry condition:
 *    Parameter is accessed for either read or write from fieldbus interface.
 *
 *   Exit condition:
 *     Parameter Index in fieldbus table is returned
 */
SINT32 GetParameterIndexInTable(UINT16 ParamNumber)
{
   ...
   if((ParamNumber >= (PARA_SET_BASE)) && ((ParamNumber <= (PARA_SET_LAST))))
    ...
    else if((ParamNumber >= (SYSCONFIG_BASE)) && ((ParamNumber <= (SYSCONFIG_LAST))))
      ParamIndexInTable = (ParamNumber - SYSCONFIG_BASE) + SysConfigBaseTableIndex;
    else
      ParamIndexInTable = -1;
 ...
 *
 *   Purpose:
 *    To check that parameter value passed in argument Value is with in limit of minimum and maximum values.
 *   Entry condition:
 *    Parameter is accessed for write from fieldbus interface.
 *
 *   Exit condition:
 *     return success if parameter values is with in range of min and max values mapped in Fiedlbus table else return failure.
 */
ParamAccessError CheckParameterRange(SINT32 ParamIndex, void * Value)
{
	ParamAccessError RetVal = VAL_ERR_NO_ERR;
	ParaType ParaTypeFB;//data type of parameter for Fieldbus interface
	//variables to keep the min, max and actual value of parameter. Keeping everything in SINT32 which is big enough to
	//hold all expected values for all parameters.
	SINT32 ParamVal = 0, ParamMinVal = 0 , ParamMaxVal = 0;
	//point to min and max values
	void * MinvalPtr = PTable[ParamIndex].MinVal, *MaxvalPtr = PTable[ParamIndex].MaxVal;

	if(FieldBusType == HW_ASSEMBLY_ETHERNETIP)
		ParaTypeFB = TYPE_S32;//everything is 32 bit for EIP
	else
		ParaTypeFB = PARATYPEPB(PTable[ParamIndex].ParamInfo);
	...
	switch(ParaTypeFB)
	{
		case TYPE_8:
		case TYPE_A_8:
			ParamVal = VAL8BE(Value);
			break;
		case TYPE_S8:
		case TYPE_A_S8:
			ParamVal = VALS8BE(Value);
			break;
		case TYPE_16:
		case TYPE_A_16:
			ParamVal = VAL16BE(Value);
			break;
		case TYPE_S16:
		case TYPE_A_S16:
			ParamVal = VALS16BE(Value);
			break;
		case TYPE_32:
		case TYPE_A_32:
			ParamVal = VAL32(Value);
			break;
		case TYPE_S32:
		case TYPE_A_S32:
			ParamVal = VALS32(Value);
			break;
		default:
			RetVal = VAL_ERR_TYPE_MISMATCH;
	};

	//now get min and max values of parameter based on actual data type
	if(RetVal == VAL_ERR_NO_ERR)
	{
		switch(PARATYPE(PTable[ParamIndex].ParamInfo))
		{
			case TYPE_8:
			case TYPE_A_8:
				if(MinvalPtr)
					ParamMinVal =  VAL8(MinvalPtr);
				if(MaxvalPtr)
					ParamMaxVal =  VAL8(MaxvalPtr);
				break;
			case TYPE_S8:
			case TYPE_A_S8:
				if(MinvalPtr)
					ParamMinVal =  VALS8(MinvalPtr);
				if(MaxvalPtr)
					ParamMaxVal =  VALS8(MaxvalPtr);
				break;
			case TYPE_16:
			case TYPE_A_16:
				if(MinvalPtr)
					ParamMinVal =  VAL16(MinvalPtr);
				if(MaxvalPtr)
					ParamMaxVal =  VAL16(MaxvalPtr);
				break;
			case TYPE_S16:
			case TYPE_A_S16:
				if(MinvalPtr)
					ParamMinVal =  VALS16(MinvalPtr);
				if(MaxvalPtr)
					ParamMaxVal =  VALS16(MaxvalPtr);
				break;
			case TYPE_32:
			case TYPE_A_32:
				if(MinvalPtr)
					ParamMinVal =  VAL32(MinvalPtr);
				if(MaxvalPtr)
					ParamMaxVal =  VAL32(MaxvalPtr);
				break;
			case TYPE_S32:
			case TYPE_A_S32:
				if(MinvalPtr)
					ParamMinVal =  VALS32(MinvalPtr);
				if(MaxvalPtr)
					ParamMaxVal =  VALS32(MaxvalPtr);
				break;
			default:
				RetVal = VAL_ERR_TYPE_MISMATCH;
		};
	...
	if(RetVal == VAL_ERR_NO_ERR)
	{
		if((MinvalPtr) && (ParamVal < ParamMinVal))
		   RetVal = VAL_ERR_LIMIT_LOW;
		 else if((MaxvalPtr) && (ParamVal > ParamMaxVal))
	   ...
	   if(SepcAcc == SPEC_ACC_ERROR)
		   RetVal = VAL_ERR_SPEC_ACCESS_MISMATCH;
			...
			RetVal = VAL_ERR_INVALID_VAL_NBR;
			 ...
			 RetVal = VAL_ERR_INVALID_VAL_NBR;
	   //check the data type width to be same as mapped in table. It can be either signed or unsigned
	   else if((PARATYPEPB(PTable[ParamIndexInTable].ParamInfo) != Type) &&
			   (PARATYPEPB(PTable[ParamIndexInTable].ParamInfo) != (Type + TYPE_OFFSET_UNSIGNEDTOSIGNED)))
			 RetVal = VAL_ERR_TYPE_MISMATCH;
	   else{
		   Access = ACCRIGHTS(PTable[ParamIndexInTable].ParamInfoExt);
		   if(!(((Access & WSAT) == WSAT) || ((Access & WS) == WS)))//check if parameter is not writable
		   		RetVal = VAL_ERR_NOT_WRITEABLE;
		   else if(((Access & WSAT) == WSAT) && (TokenAvailable == FALSE))
...
//             if(EnableDebug) pprintf("\n Non array write access");
   ...
   if((RetVal == VAL_ERR_NO_ERR) && (EnableLimitCheck == TRUE)){
       //first check the parmeter value to be in valid range
	   RetVal = CheckParameterRange(ParamIndexInTable, &TempVal);
	   if(RetVal == VAL_ERR_NO_ERR) {
	    	 ...
	    	 switch(SepcAcc){
				 case SPEC_ACC_READ_DEFAULT:
					 if((Access & RD) == RD)
					 {
						 ...
						 //if horn or weld preset are accessed allow reading default values by setting write index of
						 //preset arrays
						 if(ARRAYTYPE(PTable[ParamIndexInTable].ParamInfoExt) == ARRAYPS)
							 ArrayIndex = Preset_Default;//Point to right index for pre call back function
						 else if(ARRAYTYPE(PTable[ParamIndexInTable].ParamInfoExt) == ARRAYHSPS)
							 ArrayIndex = HsPreset_Default;//Point to right index for pre call back function
						 else
							 RetVal = VAL_ERR_SPEC_ACCESS_MISMATCH;
					 }
					 else
				 ...
				 case SPEC_ACC_READ_LIMIT_LOW:
					 if((Access & RLL) == RLL)
					 {
						 VarPtr = (UINT8*)PTable[ParamIndexInTable].MinVal;
					 	 //if horn or weld preset are accessed allow reading min values by setting write index of
						 //preset arrays
						 if(ARRAYTYPE(PTable[ParamIndexInTable].ParamInfoExt) == ARRAYPS)
							 ArrayIndex = Preset_MIN;//Point to right index for pre call back function
						 else if(ARRAYTYPE(PTable[ParamIndexInTable].ParamInfoExt) == ARRAYHSPS)
							 ArrayIndex = HsPreset_MIN;//Point to right index for pre call back function
						 else
							 RetVal = VAL_ERR_SPEC_ACCESS_MISMATCH;
					 }
					 else
				 ...
				 case SPEC_ACC_READ_LIMIT_HIGH:
					 if((Access & RLH) == RLH)
					 {
						 ...
						 //if horn or weld preset are accessed allow reading max values by setting write index of
						 //preset arrays
						 if(ARRAYTYPE(PTable[ParamIndexInTable].ParamInfoExt) == ARRAYPS)
							 ArrayIndex = Preset_MAX;//Point to right index for pre call back function
						 else if(ARRAYTYPE(PTable[ParamIndexInTable].ParamInfoExt) == ARRAYHSPS)
							 ArrayIndex = HsPreset_MAX;//Point to right index for pre call back function
						 else
							 RetVal = VAL_ERR_SPEC_ACCESS_MISMATCH;
					 }
					 else
...
//					 if(EnableDebug) pprintf("Non array read access");
   ...
   //call callback functions if applicable to format the values before sending to fieldbus interface
   if((RetVal == VAL_ERR_NO_ERR) && ((Access & RPRCB) == RPRCB))
   ...
   if(ParamIndexInTable < 0)
	...
	if ((RetVal.GSC == STAT_SUCCESS) && (EnableLimitCheck == TRUE)) {
	   if(*Len != ETHIP_PARAM_DATALEN)
	     ...
	     //first check the parmeter value to be in valid range
		 AccErr = CheckParameterRange(ParamIndexInTable, &TempVal);
		 ...
		 else if((AccErr == VAL_ERR_LIMIT_LOW) || (AccErr == VAL_ERR_LIMIT_HIGH))
			 RetVal.GSC = STAT_INVALID_PARAMETER_VALUE;//Invalid value error code as per Ethernet IP standard
		 else
		 {
			 RetVal.GSC = STAT_VENDOR_SPECIFIC_ERROR;
			 RetVal.ESC = EXTSTAT_VAL_ERR_MISC;
...
//   if(EnableDebug) printf("\n %s %d %d %d ", __FUNCTION__, VAL32(Val), Indx, RW);
   ...
   SINT32 TempSVal;
   SINT16 TempSSVal;//temporary signed short value
   static RTCTime TM = {0,0,0,0,0,0,0};
   DcxPreset * PSPtr = 0;
   HSPRESET * HspsPtr = 0;
   BOOLEAN SpecAcc = FALSE;//flag to track whether this call back is called for for user presets or special (min, max default) presets. 
   							//parameter is disabled or not is checked for only user presets based on this flag
...
// Name = Energy      
   ...
   case FB_PARAM_ENERGYCP://Energy CP special handling for rounding the value
       if(RW == ACC_WRITE){
             TempVal = VAL32(Val);
             if((TempVal > referenceForRounding) && (TempVal <= ENERGYCP_MAX)){
...
//      }
   ...
   case FB_PARAM_TIMEERRORHIGH://Time Error High (CutOff)
      if(RW == ACC_READ){
         if((SpecAcc == FALSE) && ((PSPtr->WcParam.CutOffEnabled == FALSE) || (PSPtr->WcParam.TimeCutOffFlag == FALSE))){
            ReadFromTable = FALSE;
            VAL32(Val) =  (UINT32)0;
         }
      }
...
// Step@Energy Requires Special Handling for Rounding Before Access
   ...
   case FB_PARAM_AMPLITUDEPROFILEENERGY:          
       if(RW == ACC_WRITE){
             TempVal = VAL32(Val);
             if((TempVal > referenceForRounding) && (TempVal <= AMPSTEPENERGY_MAX)){
                  if((TempVal % divisorOfReference) >= remainderForRoundingUp){
                     TempVal += (roundingUpOffset - (TempVal % divisorOfReference));
                  }
                  else{
...
// Energy Error High (CutOff)  Requires Special Handling for Rounding Before Access  
   ...
   case FB_PARAM_ENERGYERRORHIGH:
      if(RW == ACC_READ){
         if((SpecAcc == FALSE) && ((PSPtr->WcParam.CutOffEnabled == FALSE) || (PSPtr->WcParam.EnergyCutoffFlag == FALSE))){
            ReadFromTable = FALSE;
            VAL32(Val) =  (UINT32)0;
         }
      }
         ...
         TempVal = VAL32(Val);
         if((TempVal > referenceForRounding) && (TempVal <= ENERGYCUTOFF_MAX)){
              if((TempVal % divisorOfReference) >= remainderForRoundingUp){
                 TempVal += (roundingUpOffset - (TempVal % divisorOfReference));
              }
              else{
...
//*************************************************************************************
   ...
   case FB_PARAM_PEAKPOWERERRORHIGH://Peak Power Error High (CutOff)
      if(RW == ACC_READ){
         if((SpecAcc == FALSE) && ((PSPtr->WcParam.CutOffEnabled == FALSE) || (PSPtr->WcParam.PeakPowerCutoffFlag == FALSE))){
            ReadFromTable = FALSE;
            VAL32(Val) =  (UINT32)0;
         }
      }
   ...
   case FB_PARAM_TIMEERRORWINDOWLOW://Time Error Window Low
      if(RW == ACC_READ){
         if((SpecAcc == FALSE) && (((PSPtr->LimitsParam.ControlLimits & CONTROL_LIMITS_ENABLE) != CONTROL_LIMITS_ENABLE)
...
// - Energy Limit Requires Special Handling for Rounding Before Access
   ...
   case FB_PARAM_ENERGYERRORWINDOWLOW://Energy Error Window Low 
      if(RW == ACC_READ){
         ...
         TempVal = VAL32(Val);
         if((TempVal > referenceForRounding) && (TempVal <= ENERGYMLR_MAX)){
              if((TempVal % divisorOfReference) >= remainderForRoundingUp){
                 TempVal += (roundingUpOffset - (TempVal % divisorOfReference));
              }
              else{
...
// + ENERGY LIMIT Requires Special Handling for Rounding Before Access
   ...
   case FB_PARAM_ENERGYERRORWINDOWHIGH://Energy Error Window High   
      if(RW == ACC_READ){
         ...
         TempVal = VAL32(Val);
         if((TempVal > referenceForRounding) && (TempVal <= ENERGYPLR_MAX)){
              if((TempVal % divisorOfReference) >= remainderForRoundingUp){
                 TempVal += (roundingUpOffset - (TempVal % divisorOfReference));
              }
              else{
...
// **********************************************************************************************************  
   ...
   case FB_PARAM_FREQUENCYERRORLOW://Frequency Error Low (Cutoff) relative
      if(RW == ACC_READ){
         ReadFromTable = FALSE;
         if((SpecAcc == FALSE) && ((PSPtr->WcParam.CutOffEnabled == FALSE) || (PSPtr->WcParam.WeldFreqLowCutOffFlag == FALSE))){
            VAL32(Val) =  (UINT32)0;
         }
         else{
   ...
   case FB_PARAM_FREQUENCYERRORHIGH://   Frequency Error High (Cutoff) relative
      if(RW == ACC_READ){
         ReadFromTable = FALSE;
         if((SpecAcc == FALSE) && ((PSPtr->WcParam.CutOffEnabled == FALSE) || (PSPtr->WcParam.WeldFreqHighCutOffFlag == FALSE))){
            VAL32(Val) =  (UINT32)0;
         }
         else{
   ...
   case FB_PARAM_CUSTOMAIN1CUTOFFFLAG:
	  if(RW == ACC_READ){
		 if((SpecAcc == FALSE) && ((PSPtr->WcParam.CutOffEnabled == FALSE) || (PSPtr->ActParam.CustomAIn1CutoffFlag == FALSE)
   ...
   case FB_PARAM_CUSTOMAIN2CUTOFFFLAG:
	   if(RW == ACC_READ){
		 if((SpecAcc == FALSE) && ((PSPtr->WcParam.CutOffEnabled == FALSE) || (PSPtr->ActParam.CustomAIn2CutoffFlag == FALSE)
   ...
   case FB_PARAM_FBFREQOFFSET:
      if(RW == ACC_WRITE){
    	 TempSSVal = (SINT16)VALS32(Val);
         //verify the frequency specific range
         if (TempSSVal > (FbMaxPs.WcParam.FreqOffset / FREQ_SCALING_FACTOR))
            Err = VAL_ERR_LIMIT_HIGH;
         else if(TempSSVal < (FbMinPs.WcParam.FreqOffset / FREQ_SCALING_FACTOR))
            Err = VAL_ERR_LIMIT_LOW;
         else
            ExternalControl.ExtFrequencyOffset = TempSSVal;
   ...
   case FB_PARAM_EXTAMPLITUDESET:
      if(RW == ACC_WRITE){
         TempSSVal = (SINT16)VALS32(Val);
         if (TempSSVal < AMP_IN_MIN)
            Err = VAL_ERR_LIMIT_LOW;
          else if (TempSSVal > AMP_IN_MAX)
            Err = VAL_ERR_LIMIT_HIGH;
          else
             ExternalControl.ExtAmplitudeSet = TempSSVal;
...
ParamAccessError ParamAccessPostCallback(UINT16 ParamNumber, void * Val, SINT32 Indx, UINT8 RW, UINT32 * Len)
{
   ...
   case RTC_DATE:
      if(RW == ACC_WRITE) {
         if((RTDate.Date.Year > 99)|| (RTDate.Date.Month > 12) || (RTDate.Date.WeekDay > 6))
        	 Err = VAL_ERR_LIMIT_HIGH;
         else if((RTDate.Date.Month < 1)||(RTDate.Date.Date < 1))
        	 Err = VAL_ERR_LIMIT_LOW;
         //check date after checking month to avoid wrong index access in DaysinMonth array
         else if(RTDate.Date.Date > DaysInMonth(RTDate.Date.Month, RTDate.Date.Year))
            Err = VAL_ERR_LIMIT_HIGH;
      }
   break;
   case RTC_TIME:
