/*
 $Header:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/StateFunctions.cpp_v   1.1   10 Aug 2015 09:41:12   RJamloki  $
 */
/****************************************************************************/
/*                                                                          */
/*                                                                          */
...
/*************************                         **************************/
/*****************************************************************************
 $Log:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/StateFunctions.cpp_v  $
 * 
 *    Rev 1.1   10 Aug 2015 09:41:12   RJamloki
 * Changes for Clear memory with reset, clear memory before seek and clear with horn scan functionality
 * 
...
void ScanExit(void)
{
   ...
   if((ScanAborted == TRUE) || (ScanFailed == TRUE) || (MemoryValue <= 0)){
	 //  StoredFrequencyAbs[ParameterSet[CurrPreset].InfoParam.HornPresetNumber] = SCALE_UP_FOR_N_BITS(Midband);
		...
		{
		    HsParameterSet[CurrHsPreset].DigitalTuneScaledUP = SCALE_UP_FOR_N_BITS(MainParallelResonanceFreq * FREQ_SCALING_FACTOR);
			HsParameterSet[ParameterSet[CurrPreset].InfoParam.HornPresetNumber].DigitalTuneScaledUP = SCALE_UP_FOR_N_BITS(MainParallelResonanceFreq * FREQ_SCALING_FACTOR);
			Log::WriteEventLog(EVENT_PARAMETER_CHANGED,  DigitalTune, MainParallelResonanceFreq, 0);
...
void CheckForStepping(void)
{
   if(ScrubTimeActive == FALSE){
	   if(RunningPreset.WcParam.AmpStepCondition == AMPSTEP_COND_EXTERNAL){ /* If stepping on for external */
