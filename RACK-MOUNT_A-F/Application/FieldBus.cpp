/*
$Header:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/FieldBus.cpp_v   1.4.1.0   18 Feb 2016 14:01:08   ygupta  $
*/
/****************************************************************************/
/*                                                                          */
/*                                                                          */
...
/*************************                         **************************/
/*****************************************************************************
$Log:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/FieldBus.cpp_v  $
 * 
 *    Rev 1.4.1.0   18 Feb 2016 14:01:08   ygupta
 * Added virtual function ProtocolSpecificPreInit()
 * 
 *    Rev 1.4   10 Aug 2015 09:39:28   RJamloki
 * Added updates for Memory Clear signal from Fieldbus in control and status words
 * 
 *    Rev 1.3   24 Jul 2015 08:35:42   sesharma
 * Added Delay in FindFieldBusType() to give time to Hilscher Slave card to reboot.
 * 
 *    Rev 1.2   08 Jul 2015 02:41:44   sesharma
 * Added check of USActive to send Phase information zero when Sonics is not active.
 * 
 *    Rev 1.1   24 Jun 2015 08:35:32   amaurya
 * AskIPTimer initialize to default;
 * 
...
//These many retries for Hilscher Slave card detection will be done in interval of 20ms
#define SLAVE_CARDDECTECT_RETRIES 	50
...
FieldBus * FindFieldBusType()
{
	...
	//retry for some definite time to read the "netX" cookie from slave card. Retries are added
	//to give reasonable time for Hilscher slave card to boot up properly as our boot process has become
	//faster now.
	for(int i = 0; (i < SLAVE_CARDDECTECT_RETRIES) && (HilscherSlaveCardDetected == FALSE); i++)
	{
 ...
 *
 *  Purpose:
 *    To initialize global FbusIO PZD(STW) status field to be sent to Fieldbus
 *    and to determine the operating mode change.
 *    Also update External control parameter. This function is called by UpdateOutputCyclic function.
 *
 *  Entry condition:
 *    None
 *
...
//    }
   ...
   if(FBStat.UsActive == TRUE)
	   ExternalControl.Phase = (RtReceiveData.Phase * 180) / MAXDCPADCRAWVALUE;
   else
	   ExternalControl.Phase = 0; // Phase information during Sonic OFF state is send as Zero.
...
/*  void FieldBus::ProtocolSpecificPreInit(CIFXHANDLE HChannel)
 *
 *  Purpose:
 *    This function is called once at power up from FiledBusTask::Run()
...
/*Purpose: To return the currently selected horn number on RFSwitch. This function is
 * called by Statemachine.
.*	Entry Condition: None.
 *	Exit Condition: Currently selected horn on RFSwitch.
 */
...
/*Purpose: To get the amplitude control value from Fieldbus. This function is called by Statemachine
.*	Entry Condition: None.
 *	Exit Condition: None
 */
...
/*Purpose:. To get the frequency offset value from fieldbus. This function is called by Statemachine.
 * Entry Condition: None.
 * Exit Condition: None
*/
...
/*Purpose:. To get the memory clear signal status value from Fieldbus. This function is called by Statemachine.
 * Entry Condition: StateMachine.
 * Exit Condition: Memory Clear signal status is returned
*/
