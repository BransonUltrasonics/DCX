/*
 $Header:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/ProfiBus.cpp_v   1.0.1.1   18 Feb 2016 14:14:36   ygupta  $
 */
/****************************************************************************/
/*                                                                          */
/*                                                                          */
...
/*************************                         **************************/
/*****************************************************************************
 $Log:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/ProfiBus.cpp_v  $
 * 
 *    Rev 1.0.1.1   18 Feb 2016 14:14:36   ygupta
 * bus on and off is controlled by WC now
 * 
 *    Rev 1.0.1.0   21 Aug 2015 05:51:42   RJamloki
 * Fix for coverity CID 11174
 * 
...
BOOLEAN ProfiBus::GetWarmStartParams(CIFX_PACKET * WSPkt) {
   ...
   PtProfibusSetConfigReq->tData.ulSystemFlags = HOST_TO_LE32(1);//Host controlled bus start up
...
/*  void ProfiBus::ProtocolSpecificPreInit(CIFXHANDLE HChannel)
 *
 *  Purpose:
 *    This function is called once at power up from FieldbusTask::Run() function.
 *    This function implements one time protocol specific initialization before sending warmstart parameters
 *    to slave card
 *
 *  Entry condition:
 ...
 *
 *  Exit condition:
 *    profibus communication gets disabled
 */
void ProfiBus::ProtocolSpecificPreInit(CIFXHANDLE HChannel)
{
	UINT32 Temp;
	xChannelBusState(HChannel, CIFX_BUS_STATE_OFF, (uint32_t*)&Temp, 1000);
}


/*  void ProfiBus::ProtocolSpecificInit(CIFXHANDLE HChannel)
 *
 *  Purpose:
 *    This function is called once at power up from FieldbusTask::Run() function.
 *    This function implements one time protocol specific initialization after warmstart pacet is sent to slave card
 *
 *  Entry condition:
 *    HChannel:Handle to the communication channel(returned by XChannelOpen API)
 *
 *  Exit condition:
 *    profibus communication gets enabled (i.e. slave starts entertaining the parametrization ,configuration and
 *	  data exchange request from master
 *		from master)
 */
void ProfiBus::ProtocolSpecificInit(CIFXHANDLE HChannel)
{
	SINT32 Temp;
	xChannelBusState(HChannel, CIFX_BUS_STATE_ON, (uint32_t*)&Temp, 300);
}
 ...
 *	Exit Condition: None
 */
void  ProfiBus::ProcessAnalogInput(void)
{
//   static int Cntr = 0;
   SINT32 Indx = 0;
//   Cntr++;
 ...
 *	Exit Condition: None
 */
void ProfiBus::ProcessAnalogOutput(void)
{
//   static int Cntr = 0;
    SINT32 Indx = 0;
//    Cntr++;
...
ParamAccessError HandleValue(UINT16 ValNbr, ParaType Type, UINT16 ArrayIndex, UINT8 Access, void * Value) {
   ...
   case OS_SS_STATUS_SET + 8://Last SFct Number in Comm.-SSSS
      if (Type != TYPE_8)
         ...
         //code removed for coverity CID: 11174
      }
      else if (Access == ACC_WRITE)
         ReturnError = VAL_ERR_NOT_WRITEABLE;
      else
