...
/*************************                         **************************/
/*****************************************************************************
$Log:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/InputTask.cpp_v  $
 * 
 *    Rev 1.1   21 Aug 2015 05:50:56   RJamloki
 * Fixes for coverity CID 10967, 10006 ,10005 and 11343
 * 
...
//	CyGOS::DisableWatchDog();
	...
	delete[] data;//coverity CID: 10967
	delete flash;
   CyGOS::StartTimer(1);
}
...
void InputTask::ProgramCompressedTarRegionFromSerial(void)
{
	char fileName[20];
	int  ch;
	uint8 * data;
	...
	pprintf("\n invalid file length");
	delete[] data;//coverity CID: 10006
	delete flash;
   CyGOS::StartTimer(1);
}
...
//	CyGOS::DisableWatchDog();
	...
	delete[] data;//coverity CID: 10005
	delete flash;
   CyGOS::StartTimer(1);
}

void PrintDcpParameterSet(void)
{
...
void PrintCurrentParameterSet(void)
{
...
//				}
			...
			case 'D':
				...
				for(i = 0; i < sizeof(TPointsRam.TTP) / sizeof(TPointsRam.TTP[0]) ; i++){
					pprintf("\n Indx %d TTP %u", i , TPointsRam.TTP[i]);
				}
				for(i = 0; i < sizeof(TPointsRam.ITP) / sizeof(TPointsRam.ITP[0]); i++){
...
//	     	    JsonDataList.AddTail(Obj_Preset);
			...
			case 'Y':
				ssr = EnterCritical();
				pprintf("\n FRAM TTPs");
				for(i = 0; i < sizeof(DebugSection->TPoints.TTP) / sizeof(DebugSection->TPoints.TTP[0]); i++){//coverity CID:11343
