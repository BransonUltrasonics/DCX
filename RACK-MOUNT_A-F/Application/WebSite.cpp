/*
$Header:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/WebSite.cpp_v   1.4.1.3   04 Apr 2016 13:24:10   hasanchez  $
*/
/****************************************************************************/
/*                                                                          */
/*                                                                          */
...
/*************************                         **************************/
/*****************************************************************************
$Log:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/WebSite.cpp_v  $
 * 
 *    Rev 1.4.1.3   04 Apr 2016 13:24:10   hasanchez
 * Fixed order on weld results according REQ01299 and string to LCD version on system info.
 * 
 *    Rev 1.4.1.2   21 Mar 2016 16:23:24   hasanchez
 * Updated to the web page responses (string of weld results)
 * 
 *    Rev 1.4.1.1   28 Jan 2016 02:02:22   ygupta
 * ViewAlarmLogGetDatahandler() and ViewHistoryGetDatahandler() functions modified to send Power results in appropriate unit based on its selection on "System" webpage; ConfigurationSystemGetDatahandler() & ConfigurationSystemPostDatahandler() modified to handle radio button selection for unit of power result to be displayed in alarm log and history webpage.
 * 
 *    Rev 1.4.1.0   06 Nov 2015 07:02:20   sesharma
 * Fix for following issues:
 * DEF01161(Modify Profibus address on next Power cycle), 
 * DEF01164(Saving a preset), 
 * DEF01165(Amplitude changing after running auto cycle from system test page)
 * 
 *    Rev 1.4   21 Aug 2015 05:52:34   RJamloki
 * Fixes for coverity CID 11000 and 12577
 * 
 *    Rev 1.3   10 Aug 2015 09:42:52   RJamloki
 * WeldSetup page handler functions changed for addition of Clear Memory with reset, clear memory before seek and set with horn scan options.Seek and power up handler function changed for addition of clear memory at power up option
 * 
 *    Rev 1.2   24 Jul 2015 08:40:12   sesharma
 * Moved function ReadTarRegionFlash() from CyGOS to Application
 * 
 *    Rev 1.1   24 Jun 2015 08:40:26   amaurya
 * Added check for system type change and update LCD reg
 * 
...
void WebSite::InitWebSite(SINT32 Intf, SINT32 SrvrPort){
   ...
   *TempParameterSet = ParameterSet[CurrPreset];
   CyGOS::AddToUploadSupportList(Intf, SrvrPort, "bin" , (SINT32)__FLASH_SIZE + (SINT32)__BOOTLOADER_SIZE + (SINT32)__TAR_SIZE);
   CyGOS::AddToUploadSupportList(Intf, SrvrPort, "cyg" ,  (SINT32)__TAR_SIZE);
   CyGOS::AddToUploadSupportList(Intf, SrvrPort, "cfg" ,  (SINT32)FRAM_SIZE);
   CyGOS::AddToSendBackUploadDataList(Intf, SrvrPort, "SendBackCSVGraphDataPS","Welddata.csv");
   CyGOS::AddToSendBackUploadDataList(Intf, SrvrPort, "SendBackCSVGraphDataHS","HornScanData.csv");
   CyGOS::SetFileUploadSuccessMsg(Intf, DCXWEBSITE_PORT, (char *)FileUploadSuccessMsg);
}
...
void WebSite::SetupSeekPowerUpGetDatahandler(SINT8 * ParamStr, WebCommand Cmd)
{
   ...
   sprintf(&Response[strlen(Response)],
      ...
      Sysconfig->ClearMemoryAtPowerUp);
...
void WebSite::SetupSeekPowerUpPostDatahandler(SINT8 * ParamStr, WebCommand Cmd)
{
   ...
   if(TokenAvailable){
      ...
      //Clear Memory at Power Up Option
      TempStr = strstr(ParamStr, "&ClearMemAtPowerUp=");
      Sysconfig->ClearMemoryAtPowerUp = atoi(strchr(TempStr, '=') + 1);     
...
void WebSite::ViewSystemInfoGetDatahandler(SINT8 * ParamStr, WebCommand Cmd)
{
    ...
    const SINT8 * DispInfo = "OK";
 	...
 	sprintf(&Response[strlen(Response)],
      ...
      DispInfo,
      //(ReceiveCommandHandler::LCDSWVersion / 100),((ReceiveCommandHandler::LCDSWVersion / 10)%10),(ReceiveCommandHandler::LCDSWVersion % 10) ,
      LCDcrc,
 	...
 	if (ReceiveCommandHandler::LCDSWVersion / 100 >= 1)  // diivision to know if the Version on on the range of hundreds.
 	{
 		sprintf(&Response[strlen(Response)],
 		"@LCDswversion:S,%d.%d.%d",
 		(ReceiveCommandHandler::LCDSWVersion / 100),((ReceiveCommandHandler::LCDSWVersion / 10)%10),(ReceiveCommandHandler::LCDSWVersion % 10));
 	...
 	else
 	{
 		sprintf(&Response[strlen(Response)],
 		"@LCDswversion:S,%d.%d",
 		(ReceiveCommandHandler::LCDSWVersion / 10),(ReceiveCommandHandler::LCDSWVersion % 10) );
 	}
...
void WebSite::ViewAlarmLogGetDatahandler (SINT8 * ParamStr, WebCommand Cmd)
{
   ...
   DcxLogUserID = 0;
   SINT8 * AlarmPwr = new SINT8[strlen("Alarm Power")+strlen(PowerUnit[0])];
   strcpy(AlarmPwr, "Alarm Power");
   strcat(AlarmPwr,PowerUnit[Sysconfig->PowerResUnit]);

   SINT8 * WeldPwr = new SINT8[strlen("Power")+strlen(PowerUnit[0])];
   strcpy(WeldPwr, "Power");
   strcat(WeldPwr, PowerUnit[Sysconfig->PowerResUnit]);
   if(Download){
      sprintf(&Response[strlen(Response)],"%s,%s,%s,%s,%s,%s","Alarm #",
    		  "Date & Time,Cycle Num,Alarm,Alarm Code,Alarm Freq",
    		   AlarmPwr,
    		  "Alarm Amp,Alarm Current,Weld Mode,Weld Time,Weld Energy",
    		   WeldPwr,
    		  "Amp1,Amp2,"
      ...
      sprintf(&Response[strlen(Response)], "%s$LOG, %s,", "alarmLog", PowerUnit[Sysconfig->PowerResUnit]);
      strcpy(DataFormat, "%d*%02d-%02d-%02d  %s*%d*%s*%03X*%d*%d*%d*%d*%s*%d*%d.%d*%d*%d*%d*%d*%d*%d*%d*%d*%d*%d*%d.%d*%d.%d#");
   }
   ...
   for(Index = 0; Index < LogCount; Index++){
         ...
         if(Sysconfig->PowerResUnit == PowerInWatts){  //Convert Power in Watt based on Power supply rating
        	 Power = (WebAlarmdata[Index].AlarmData.WeldPower * MacchipData.Power)/SCALING_FACTOR;
        	 ErrorPower = (WebAlarmdata[Index].AlarmData.ErrorPower * MacchipData.Power)/SCALING_FACTOR;
         }
         else if(Sysconfig->PowerResUnit == PowerInPercent){
             Power = WebAlarmdata[Index].AlarmData.WeldPower;
             ErrorPower = WebAlarmdata[Index].AlarmData.ErrorPower;
         }
...
void WebSite::ViewHistoryGetDatahandler(SINT8 * ParamStr, WebCommand Cmd)
{
   ...
   DcxLogUserID = 0;
   SINT8 * PeakPwr = new SINT8[strlen("Peak Power")+strlen(PowerUnit[0])];
   strcpy(PeakPwr, "Peak Power");
   strcat(PeakPwr, PowerUnit[Sysconfig->PowerResUnit]);
   if(Download){
       sprintf(&Response[strlen(Response)], "%s,%s,%s,%s", "Cycle #", "Date & Time,Weld Mode,Weld Time,Weld Energy",
    		   PeakPwr,
    		   "Amp1, Amp2, Preset No., Temp P/S(ºC), Start Freq, End Freq, Freq Change, AlarmCode, CustomIn1, CustomIn2\r\n");
       strcpy(DataFormat, "%d, %02d-%02d-%02d  %s,%s,%d.%03d,%d.%d,%d,%s,%s,%d,%d,%d,%d,%d,%s,%d.%d,%d.%d\r\n");
   }
   else{
       sprintf(&Response[strlen(Response)], "%s$HISTORY, %s,", "HistoryLog", PowerUnit[Sysconfig->PowerResUnit]);
      // strcpy(DataFormat, "%d*%02d-%02d-%02d  %s*%s*%d.%03d*%d.%d*%d^%s*%s*%s*%d*%d*%d*%d*%s*%d.%d*%d.%d#");
       strcpy(DataFormat, "%d*%02d-%02d-%02d  %s*%s*%d.%03d*%d.%d*%d*%s*%s*%d*%d*%d*%d*%d*%s*%d.%d*%d.%d#");
   }
   for(Indx = 0; Indx < LogCount; Indx++){
      ...
      if((WeldMode < 0) || (WeldMode > 4))
    	  WeldMode = 5;
      if(Sysconfig->PowerResUnit == PowerInWatts){
     	 PeakPower = (WeldHistory[Indx].PeakPower * MacchipData.Power)/SCALING_FACTOR;	//Convert Peak Power in Watt based on Power supply rating
      }
      else if(Sysconfig->PowerResUnit == PowerInPercent)
         PeakPower = WeldHistory[Indx].PeakPower;
      //pprintf("\n History Index %d\n", Indx);
      sprintf(&Response[strlen(Response)], DataFormat,
         WeldHistory[Indx].CycleCounter,
         (SINT32)Temp1, (SINT32)Temp2, WeldHistory[Indx].TS.Year, TempTime, WMode[(UINT8)WeldMode],
         (WeldHistory[Indx].WeldTime/1000),(WeldHistory[Indx].WeldTime%1000), WeldHistory[Indx].WeldEnergy/ENERGY_CALCULATION_FACTOR,WeldHistory[Indx].WeldEnergy%ENERGY_CALCULATION_FACTOR,
         PeakPower,
         AmpStr1, AmpStr2,
         WeldHistory[Indx].PresetNumber, WeldHistory[Indx].Temperature, WeldHistory[Indx].WeldStartFrequency, WeldHistory[Indx].WeldStopFrequency, FreqChange, ErrReason,
...
//)
   if((TRUE == UserIO::GetAlarmStatus()) ||
		   (Sysconfig->ClearMemoryWithReset && (TRUE == UserIO::GetReadyFlag()))) {
      resetFlag = TRUE;
...
void WebSite::DiagnosticFieldBusGetDatahandler(SINT8 * ParamStr, WebCommand Cmd)
{
   ...
   if(Dummy == HW_ASSEMBLY_ETHERNETIP){
      ...
      EthIPSlaveAddr.addr = EthIPConfig.IpAddr;//always show the actual slave IP on diagnostic page
      strcpy(SlaveAddr, EthIPSlaveAddr.ToString());
   }
   else if(Dummy == HW_ASSEMBLY_PROFIBUS){
...
void WebSite::ConfigurationSystemPostDatahandler(SINT8 * ParamStr, WebCommand Cmd)
{
   if(TokenAvailable){
      ...
      TempStr = strstr(ParamStr, "&PowerUnit=");
      Sysconfig->PowerResUnit = atoi(strchr(TempStr, '=') + 1);
      pprintf("\nSet Power Unit = %d", Sysconfig->PowerResUnit);
      RegUpdateCmdFlag = TRUE;
      strcpy(Response,"OK");
   }
   Cmd = REQ_NONE;
      ...
      case REQ_MFPOLL:
         if(CurrIndx == ERROR_IDX)
 		 ...
 		 if(CurrentCycleParam->TotalCycleTime > 0){
			 ...
			 if(MFcycleResults->Cyclestatus != STATECOMPLETED)
				 RemainingCycleTime += MSINMIN;
			 else if(MFcycleResults->Cyclestatus == STATECOMPLETED)
				 RegAmplitudeUpdateFlag = TRUE;	//Update Amplitude on LCD after cycle has completed
 		 }
...
void WebSite::ProcessMFSavesystemSetupCommand(SINT8 * Param)
{
   ...
   if(MacchipData.CurrSystemType != (SYSTEMTYPE)(atoi(Temp))){
...
SINT8 * WebSite::FindLanguageString(UINT32 StringId, SINT8 * FileBuff, BOOLEAN StrOnly)
{
   ...
   if(!StrOnly){
      ...
      if(strlen(LangString) < MAX_LANGUAGESTRING_LEN - 1 )
         strncpy(LangString, strchr(LangString, ',') + 1, MAX_LANGUAGESTRING_LEN - 1);//coverity CID: 11000
   }
   return LangString;
}
...
//		}
		...
		delete[] TarImage;//coverity CID: 12577
