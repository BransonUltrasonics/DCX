 ...
 **************************************************************************************
   ...
   Filename:
    $Id:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/CifXToolkit/Source/cifXHWFunctions.cpp_v   1.0   27 Jul 2015 08:55:32   sesharma  $
   Last Modification:
    $Author:   sesharma  $
    $Date:   27 Jul 2015 08:55:32  $
    $Revision:   1.0  $
...
void DEV_ReadHandshakeFlags(PCHANNELINSTANCE ptChannel, int fReadSyncFlags, int fLockNeeded)
{
  ...
  if(ptChannel->fIsSysDevice)
  {
    ...
    pulCOSFlags = &ptSysChannel->tSystemState.ulSystemCOS;
  } else
  {
    ...
    if(NULL != ptChannel->ptCommonStatusBlock)
      pulCOSFlags = &ptChannel->ptCommonStatusBlock->ulCommunicationCOS;
  }

  if(ptChannel->bHandshakeWidth == RCX_HANDSHAKE_SIZE_8BIT)
  {
    /* Read 8 Bit handshake */
    ptChannel->usNetxFlags = (uint8_t)(HWIF_READ8(ptDevInstance, ptChannel->ptHandshakeCell->t8Bit.bNetxFlags));
  } else
  {
...
int DEV_IsRunning(PCHANNELINSTANCE ptChannel)
{
  ...
  if(!ptChannel->fIsSysDevice)
  {
	uint32_t DevCosFlags = ptChannel->ulDeviceCOSFlags;
    if( (DevCosFlags & RCX_COMM_COS_READY) &&
        (DevCosFlags & RCX_COMM_COS_RUN) )
    {
      iRet = 1;
    }
    else
    {
    	...
    	{
    		FbFailCounter[2]++;//0x00000001
    		FbLastErrorReason = 3;
    	}
    	if(DevCosFlags & RCX_COMM_COS_RUN){}
    	else
    	{
    		FbFailCounter[3]++;//0x00000002
    		FbLastErrorReason = 4;
    	}
    }
  }
...
int DEV_IsCommunicating(PCHANNELINSTANCE ptChannel, int32_t* plError)
{
  ...
  {
     NetXFlags = ptChannel->usNetxFlags;
    /* only a Communication channel can be running */
    // pprintf(" %04X %d ", ptChannel->fIsSysDevice, (NetXFlags & NCF_COMMUNICATING));
    if( (!ptChannel->fIsSysDevice) &&
        (NetXFlags & NCF_COMMUNICATING))
    {
      iRet = 1;
...
//                TRACE_LEVEL_DEBUG,
//    	pprintf("DEV_CheckCOSFlags(): Skipping COS Flag handling. Device is in system reset!");
    }
  } else
  {
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_DEBUG,
//        	pprintf("DEV_CheckCOSFlags(): Skipping Channel #%d, which is currently initializing!",
//                    ulChannel);
        }
...
//                TRACE_LEVEL_ERROR,
//    	pprintf("DEV_DoChannelInit(): Error getting Mutex. Access to device COS flags is locked!");
    }
...
//            USER_Trace(ptDevInst,
//                      TRACE_LEVEL_DEBUG,
//        	  pprintf("DEV_DoChannelInit(): Firmware removed RCX_COMM_COS_RUN early! Skipping wait for NotRunning-State");
          }
...
//            USER_Trace(ptDevInst,
//                      TRACE_LEVEL_ERROR,
//        	  pprintf("DEV_DoChannelInit(): Error waiting for channel to leave running state!");
          }
        }
      }
...
//                           TRACE_LEVEL_WARNING,
//            	  pprintf("DEV_DoChannelInit(): Channel did not enter READY state during timeout!");
              }
            }
          }
        }
...
//      USER_Trace(ptDevInstance,
//                TRACE_LEVEL_ERROR,
//    	pprintf("DEV_DoSystemStart(): Error locking access to device!");
    }
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
//    	  pprintf("DEV_DoSystemStart(): Error waiting for device to leave READY state!");
      }
    }
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_ERROR,
//        	pprintf("DEV_DoSystemStart(): Error waiting for device to become ready!");
        }
        /* Reset failed */
...
//      USER_Trace(ptDevInstance,
//                TRACE_LEVEL_ERROR,
//    	pprintf("DEV_DoSystemBootstart(): Error locking access to device!");
    }
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
//    	  pprintf("DEV_DoSystemBootstart(): Error waiting for device to leave READY state!");
      }
    }
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_ERROR,
//        	pprintf("DEV_DoSystemBootstart(): Error waiting for device to become ready!");
        }
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
//        	  pprintf("DEV_DoSystemBootstart(): Error setting card into boot mode!");
          }
...
//  int32_t       lCount           = 0;
  unsigned long ulSourceID       = 0x43214321;
  ...
  tSendPacket.tHeader.ulCmd  = HOST_TO_LE32(0x00002F14); /* delete configuration CMD */

  /* send packet */
  lRet = DEV_TransferPacket( ptChannel, &tSendPacket, &tReceivePacket,
                             sizeof(tReceivePacket), CIFX_TO_SEND_PACKET, NULL, NULL);
  if ( (CIFX_NO_ERROR == lRet)                                    &&
       (CIFX_NO_ERROR == (lRet = tReceivePacket.tHeader.ulState)) )
