                       ...
                       void*                  pvUser)
{
  ...
  memset(&uSendPkt, 0, sizeof(uSendPkt));
...
{
  int32_t         lRet          = CIFX_NO_ERROR;
//  PDEVICEINSTANCE ptDevInstance = (DEVICEINSTANCEtag*)((PCHANNELINSTANCE)pvChannel)->pvDeviceInstance;
...
//                 TRACE_LEVEL_ERROR,
//                 pprintf("Failed to send MD5 request, lRet = 0x%08x", lRet);
    }
...
//      USER_Trace(ptDevInstance,
//                 TRACE_LEVEL_INFO,
//                 pprintf("No MD5 Information available. Probably the file does not exist on device. (ulState = 0x%08x)",
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_INFO,
//                  pprintf("MD5 checksum is identical, download not necessary");
      }
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_INFO,
//                  pprintf("MD5 not identical, process firmware download");
      }
    }
  }
...
//          USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
//        	pprintf("Error channel number %u for a firmware is not supported",
//                      ulChannel);
        }
...
//          USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
//        	pprintf("Error NXO files are not allowed without a Base OS firmware");
        }
      } else
      {
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_DEBUG,
//        	  pprintf("Successfully downloaded the firmware to device '%s'!",
//                      pszFullFileName);
          }
        }
      }
...
//                          ulChannel);
          ...
          } else
          {
            ...
            {
              DEV_RemoveChannelFiles((CHANNELINSTANCEtag*)hSysDevice, ulChannel, pfnTransferPacket, NULL, NULL, NULL);
            }
...
//            	pprintf("Error NXO files are not allowed without a Base OS firmware");
          ...
          } else
          {
            ...
            /* We have to delete existing files, depending of a NXF/NXO */
            /* Leave NXF file */
            DEV_RemoveChannelFiles( (CHANNELINSTANCEtag*)hSysDevice, ulChannel, pfnTransferPacket, NULL, NULL, HIL_FILE_EXTENSION_FIRMWARE);
...
//        USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_ERROR,
//    	  pprintf("Error unsupported device type %u found for download handling!",
                         ...
                         void*                 pvUser)
{
  ...
  memset(&uRecvPkt, 0, sizeof(uRecvPkt));
