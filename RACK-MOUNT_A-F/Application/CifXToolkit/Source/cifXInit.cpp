 ...
 **************************************************************************************
   ...
   Filename:
    $Id:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/CifXToolkit/Source/cifXInit.cpp_v   1.1   21 Aug 2015 05:45:12   RJamloki  $
   Last Modification:
    $Author:   RJamloki  $
    $Date:   21 Aug 2015 05:45:12  $
    $Revision:   1.1  $
...
**************************************************************************************
$Log:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/CifXToolkit/Source/cifXInit.cpp_v  $
 * 
 *    Rev 1.1   21 Aug 2015 05:45:12   RJamloki
 * Fixes for coverity CID 10952 and 11203
*/
...
TKIT_DRIVER_INFORMATION g_tDriverInfo   = {0, 0};    /*!< Global driver information            */
...
void IntHandlerExternal(int level, void * frame)
{
	...
	if((cifXTKitISRHandler( g_pptDevices[g_ulDeviceCount - 1], 1)) == CIFX_TKIT_IRQ_DSR_REQUESTED)
		cifXTKitDSRHandler(g_pptDevices[g_ulDeviceCount - 1]);
//	pprintf("\n ext int 6 %d %08X",  g_pptDevices[g_ulDeviceCount - 1]->fIrqEnabled, g_pptDevices[g_ulDeviceCount - 1]);
}
...
static int32_t cifXHardwareReset(PDEVICEINSTANCE ptDevInstance)
{
 ...
 if(pvPCIConfig){
	...
	//                   TRACE_LEVEL_ERROR,
			  pprintf("DPM Content invalid after Reset (Data=0x%08X)!",
					   ulState);
		  }
		  break;
                                        ...
                                        uint32_t   ulFileDataLen)
{
  ...
  pbTmp = (uint8_t*)OS_Memalloc(ulFileDataLen);
...
//      USER_Trace(ptDevInstance,
//                TRACE_LEVEL_ERROR,
//    	pprintf("Downloading of bootloader to DPM failed!");
    }
...
//    USER_Trace(ptDevInstance,
//               TRACE_LEVEL_DEBUG,
    pprintf("Found netX50 ROMloader, DPM layout type 0x%08X",
               ulLayout);
  }
...
//    USER_Trace(ptDevInstance,
//               TRACE_LEVEL_ERROR,
	  pprintf("Invalid Bootloader file. File must be larger than 64 Bytes. (Detected Size:%u)",
                ulFileDataLen);
...
//      USER_Trace(ptDevInstance,
//                TRACE_LEVEL_ERROR,
    	pprintf("Error transfering bootheader to netX50 Bootloader (lRet = 0x%08X)",
                 lRet);
...
//      USER_Trace(ptDevInstance,
//                TRACE_LEVEL_ERROR,
    	pprintf("netX50 ROMloader rejected bootblock (bResult = %u)",
                 bResult);
      lRet = CIFX_DRV_DOWNLOAD_FAILED;
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
    	  pprintf("Error transferring data packet from/to netX50 Bootloader (lRet = 0x%08X)",
                   lRet);
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_ERROR,
        	pprintf("Error getting final packet from netX50 ROM Loader. lRet=0x%08X",
                    lRet);
        }
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_ERROR,
        	pprintf("netX50 ROM Loader download error. bResult=%u",
                    bResult);
        }
        lRet = CIFX_DRV_DOWNLOAD_FAILED;
...
//      USER_Trace(ptDevInstance,
//                TRACE_LEVEL_ERROR,
//    	pprintf("Error opening bootloader file '%s'!",
...
//      USER_Trace(ptDevInstance,
//                 TRACE_LEVEL_INFO,
//    	pprintf("Downloading bootloder '%s'",
//                 tFileInfo.szFullFileName);
    }
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
//    	  pprintf("Error creating file buffer!");
      }
    } else
    {
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_ERROR,
        	pprintf("Error reading bootloader file '%s'!",
                    tFileInfo.szFullFileName);
        }
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_DEBUG,
        	  pprintf("Calling supplied function (0x%08X) before starting bootloader!",
...
//              USER_Trace(ptDevInstance,
//                        TRACE_LEVEL_DEBUG,
            	pprintf("Calling supplied function (0x%08X) after starting bootloader!",
                        ptDevInstance->pfnNotify);
            }
...
//                  USER_Trace(ptDevInstance,
//                            TRACE_LEVEL_DEBUG,
//                	pprintf("Bootloader was downloaded and started successfully!");
                }
...
//              USER_Trace(ptDevInstance,
//                        TRACE_LEVEL_ERROR,
            	pprintf("DPM not accessible after starting Bootloader! (lRet=0x%08X)",
                        lRet);
            }
          }
        }
...
//      USER_Trace(ptDevInstance,
//                TRACE_LEVEL_ERROR,
    	pprintf("RAM based device needs a DPM >= 64kB to work (DPMSize=%u). Device cannot be handled!",
...
//      USER_Trace(ptDevInstance,
//                TRACE_LEVEL_DEBUG,
//    	pprintf("New RAM based device found, device will be reset!");
    }
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_DEBUG,
    	  pprintf("Calling supplied function (0x%08X) before resetting the card (HWReset)!",
                  ptDevInstance->pfnNotify);
      }
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
    	  pprintf("Hardware reset failed, check if the card is correctly configured (PCI/DPM bootmode) (lRet=0x%08X)!",
                  lRet);
      }
    } else
    {
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_DEBUG,
        	pprintf("Calling supplied function (0x%08X) after resetting the card!",
                    ptDevInstance->pfnNotify);
        }
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_ERROR,
        	pprintf("Bootloader could not be started! (lRet=0x%08X)",
                    lRet);
        }
      }
    }
...
static int32_t cifXHandleRAMBaseOSModule(PDEVICEINSTANCE ptDevInstance)
{
  ...
  int32_t                 lRet      = CIFX_NO_ERROR;
  CIFX_DEVICE_INFORMATION tDevInfo  = {0 , 0, 0 , 0};
  CIFX_FILE_INFORMATION   tFileInfo = {{0},{0}};
...
//      USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_WARNING,
    	pprintf("O/S file (%s) found. Download and start base rcX System!", tFileInfo.szShortFileName);
    }
...
//        USER_Trace(ptDevInstance,
//                   TRACE_LEVEL_ERROR,
    	  pprintf("Error opening OS file '%s'!",
                   tFileInfo.szFullFileName);
      }
...
//          USER_Trace( ptDevInstance,
//                      TRACE_LEVEL_ERROR,
        	pprintf("Error reading OS file from disk '%s'!",
                      tFileInfo.szFullFileName);
        }
...
//            USER_Trace( ptDevInstance,
//                        TRACE_LEVEL_DEBUG,
        	  pprintf("OS file was downloaded successfully '%s'", tFileInfo.szFullFileName);
          }
...
//              USER_Trace(ptDevInstance,
//                        TRACE_LEVEL_ERROR,
            	pprintf("Error sending Start request to start Base OS (lRet=0x%08X)!",
                        lRet);
            }
          }  else
          {
...
//                USER_Trace(ptDevInstance,
//                          TRACE_LEVEL_ERROR,
            	  pprintf("Error waiting for firmware to leave reset state! (lRet=0x%08X)",
                          lRet);
              }
            } else
            {
...
//                  USER_Trace(ptDevInstance,
//                            TRACE_LEVEL_ERROR,
                	pprintf("Error device does not reach READY state! (lRet=0x%08X)",
                            lRet);
                }
              }
            }
...
static int32_t cifXStartFlashDevice(PDEVICEINSTANCE ptDevInstance)
{
  ...
  pprintf("\n %s cookie-%s",__FUNCTION__, szCookie);
  return lRet;
}
...
////            USER_Trace( ptDevInstance,
////                        TRACE_LEVEL_ERROR,
//        	  pprintf("Error checking for download '%s'!",
//                        tFileInfo.szFullFileName);
//          }
//        } else if (!fDownload)
//        {
...
//          {
//            DEV_RemoveChannelFiles( (CHANNELINSTANCEtag*)hSysDevice, ulChNum, DEV_TransferPacket, NULL, NULL, NULL);
//          }
//
...
////              USER_Trace( ptDevInstance,
////                          TRACE_LEVEL_DEBUG,
//            	pprintf("OS file was downloaded successfully '%s'\r\n", tFileInfo.szFullFileName);
//            }
//
//            /* Start the Base OS */
...
////                USER_Trace(ptDevInstance,
////                          TRACE_LEVEL_ERROR,
//            	  pprintf("Error during Flash based Base OS system start! (lRet=0x%08X)",
//                          lRet);
//              }
//            }
//          }
...
//      USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_INFO,
    	pprintf("Firmware download, checking / starting: CHANNEL #%d, %d file(s)",
                  ulChannel,
                  ulFirmwareCnt);
    }
...
//          USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
        	pprintf("Error querying Firmware to load from USER_GetFirmwareFile (ulIdx=%u)!",
                      ulIdx);
        }
      } else
      {
...
//            USER_Trace(ptDevInstance,
//                        TRACE_LEVEL_ERROR,
        	  pprintf("Error opening Firmware file '%s'!",
                        tFileInfo.szFullFileName);
          }
...
//              USER_Trace(ptDevInstance,
//                        TRACE_LEVEL_ERROR,
            	pprintf("Error reading Firmware file from disk '%s'!",
                        tFileInfo.szFullFileName);
            }
          } else
          {
...
//      USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_INFO,
//    	pprintf("Configuration download, checking / starting: CHANNEL#%d, %d file(s)!",
//                  ulChannel,
//                  ulConfigCnt);
    }
...
//          USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
//        	pprintf("Error querying configuration to load via USER_GetConfigurationFile (ulIdx=%u)!",
//                      ulIdx);
        }
      } else
      {
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
        	  pprintf("Error opening configuration file '%s'!",
                      tFileInfo.szFullFileName);
          }
        } else
        {
...
//              USER_Trace(ptDevInstance,
//                        TRACE_LEVEL_ERROR,
//            	pprintf("Error reading configuration file from disk '%s'!",
//                        tFileInfo.szFullFileName);
            }
          } else
          {
...
//                  USER_Trace(ptDevInstance,
//                            TRACE_LEVEL_DEBUG,
//                	pprintf("Successfully downloaded the configuration to device '%s'!",
//                            tFileInfo.szFullFileName);
                }
              }
            }
...
//      USER_Trace(ptDevInstance,
//                TRACE_LEVEL_WARNING,
    	pprintf("Error querying firmware information! (lRet=0x%08X)",
                         ...
                         uint32_t ulFileSize, PFN_RECV_PKT_CALLBACK pfnRecvPktCallback, void* pvUser)
{
  ...
  memset(&uSendPacket, 0, sizeof(uSendPacket));
...
//      USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
    	pprintf("Error starting module '%s' on Channel %d - (lRet=0x%08X)!",
                  pszModuleName,
                  ulChannelNumber,
                  lRet);
    }
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
    	  pprintf("Error sending Start request for Firmware (lRet=0x%08X)!",
                  lRet);
      }
    } else
    {
...
//          USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
        	pprintf("System error information, (SystemError=0x%08X)!",
                      ulError);
        }
...
//          USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
        	pprintf("System state information, (SystemState=0x%08X)!",
                      ulError);
        }
      }
    }
...
//      USER_Trace(ptDevInstance,
//                TRACE_LEVEL_DEBUG,
//    	pprintf("System channel is READY!");
    }
  }
...
////              USER_Trace(ptDevInstance,
////                        TRACE_LEVEL_ERROR,
//            	pprintf("Error during system start! (lRet=0x%08X)",
//                        lRet);
//            }
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_DEBUG,
        	  pprintf("Successfully performed channel init on channel #%d!",
                      ulChannel);
          }
        } else
        {
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
        	  pprintf("Error performing channel init on channel #%d (lRet=0x%08X)!",
                      ulChannel,
                      lChannelRet);
          }
        }
...
static int32_t cifXCreateSystemDevice(PDEVICEINSTANCE ptDevInstance)
{
  ...
  uint32_t sizeDPM = LE32_TO_HOST(HWIF_READ32(0, ptSysChannel->tSystemInfo.ulDpmTotalSize));

  pprintf(" size read from system channel system information block %08X", sizeDPM);
...
//    USER_Trace(ptDevInstance,
//              TRACE_LEVEL_DEBUG,
	  pprintf(" - Device Number : %u",
              ptDevInstance->ulDeviceNumber);

//    USER_Trace(ptDevInstance,
//              TRACE_LEVEL_DEBUG,
	  pprintf(" - Serial Number : %u",
              ptDevInstance->ulSerialNumber);

//    USER_Trace(ptDevInstance,
//              TRACE_LEVEL_DEBUG,
	  pprintf(" - Slot Number   : %u",
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_WARNING,
        	pprintf("Duplicate alias '%s' passed (DevNr=%u, SerNr=%u), Alias will be removed!",
...
//    USER_Trace(ptDevInstance,
//                TRACE_LEVEL_DEBUG,
	  pprintf("Reading Channel Info on Channel#%d (DPM Start Offset=0x%08X Length=0x%08X)",
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
//    	  pprintf("Error reading subblock information, Error: 0x%08X  (AreaIndex=%d, SubblockIndex=%d)",
//                  lRet,
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
//    	  pprintf("Error reading subblock information, error firmware answer,  Error: 0x%08X  (AreaIndex=%d, SubblockIndex=%d)",
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_DEBUG,
//        	pprintf("Undefined/Unknown subblock type (Channel=%d, Block=%d, Type=0x%08X)",
//                    ptChannel->ulChannelNumber,
//                    LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
...
//              pprintf("\n mem realloc %s  ptChannel->pptIOOutputAreas = %08X", __FUNCTION__, ptChannel->pptIOOutputAreas);
...
//                USER_Trace(ptDevInstance,
//                          TRACE_LEVEL_DEBUG,
//            	  pprintf("I/O Output Subblock found    (Channel=%d, Block=%d, Offset=0x%08X, Len=0x%04X)",
//                          ptChannel->ulChannelNumber,
//                          LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
...
//                          LE32_TO_HOST(tRecvPkt.tData.ulSize));
            ...
            case RCX_DIRECTION_IN:
            {
              ...
              //pprintf("\n mem realloc %s  ptChannel->pptIOInputAreas = %08X", __FUNCTION__, ptChannel->pptIOInputAreas);
              ptChannel->pptIOInputAreas = (IOINSTANCEtag**)OS_Memrealloc(ptChannel->pptIOInputAreas,
...
//                USER_Trace(ptDevInstance,
//                          TRACE_LEVEL_DEBUG,
//            	  pprintf("I/O Input Subblock found     (Channel=%d, Block=%d, Offset=0x%08X, Len=0x%04X)",
//                          ptChannel->ulChannelNumber,
//                          LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
//                          LE32_TO_HOST(tRecvPkt.tData.ulOffset),
//                          LE32_TO_HOST(tRecvPkt.tData.ulSize));
...
//                USER_Trace(ptDevInstance,
//                          TRACE_LEVEL_ERROR,
//            	  pprintf("Invalid I/O direction found! (Channel=%d, Block=%d,Dir=0x%08X)",
//                          ptChannel->ulChannelNumber,
//                          LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
...
//                USER_Trace(ptDevInstance,
//                          TRACE_LEVEL_DEBUG,
//            	  pprintf("Output Mailbox found         (Channel=%d, Block=%d, Offset=0x%08X, Len=0x%04X)",
//                          ptChannel->ulChannelNumber,
//                          LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
//                          LE32_TO_HOST(tRecvPkt.tData.ulOffset),
//                          LE32_TO_HOST(tRecvPkt.tData.ulSize));
...
//                USER_Trace(ptDevInstance,
//                          TRACE_LEVEL_DEBUG,
//            	  pprintf("Input Mailbox found          (Channel=%d, Block=%d, Offset=0x%08X, Len=0x%04X)",
//                          ptChannel->ulChannelNumber,
//                          LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
//                          LE32_TO_HOST(tRecvPkt.tData.ulOffset),
//                          LE32_TO_HOST(tRecvPkt.tData.ulSize));
...
//                USER_Trace(ptDevInstance,
//                          TRACE_LEVEL_ERROR,
//            	  pprintf("Invalid mailbox direction found! (Channel=%d, Block=%d,Dir=0x%08X)",
//                          ptChannel->ulChannelNumber,
//                          LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
//                          LE16_TO_HOST(tRecvPkt.tData.usFlags) & RCX_DIRECTION_MASK);
              }
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_DEBUG,
//        	  pprintf("Control block found          (Channel=%d, Block=%d, Offset=0x%08X, Len=0x%04X)",
//                      ptChannel->ulChannelNumber,
//                      LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
//                      LE32_TO_HOST(tRecvPkt.tData.ulOffset),
//                      LE32_TO_HOST(tRecvPkt.tData.ulSize));
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_DEBUG,
        	  pprintf("Common Status block found    (Channel=%d, Block=%d, Offset=0x%08X, Len=0x%04X)",
                      ptChannel->ulChannelNumber,
                      LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
                      LE32_TO_HOST(tRecvPkt.tData.ulOffset),
                      LE32_TO_HOST(tRecvPkt.tData.ulSize));
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_DEBUG,
        	  pprintf("Extended Status block found  (Channel=%d, Block=%d, Offset=0x%08X, Len=0x%04X)",
                      ptChannel->ulChannelNumber,
                      LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
                      LE32_TO_HOST(tRecvPkt.tData.ulOffset),
                      LE32_TO_HOST(tRecvPkt.tData.ulSize));
        ...
        case RCX_BLOCK_USER:
        {
          ...
          pprintf("\n mem realloc %s  ptChannel->pptUserAreas = %08X", __FUNCTION__, ptChannel->pptUserAreas);
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_DEBUG,
        	  pprintf("User block found             (Channel=%d, Block=%d, Offset=0x%08X, Len=0x%04X)",
                      ptChannel->ulChannelNumber,
                      LE32_TO_HOST(tSendPkt.tData.ulSubblockIndex),
                      LE32_TO_HOST(tRecvPkt.tData.ulOffset),
                      LE32_TO_HOST(tRecvPkt.tData.ulSize));
...
//while(1);
 // pprintf(" return %s %d", __FUNCTION__, lRet);
  return lRet;
}
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_DEBUG,
//    	  pprintf("No warm start parameter found or available!");
      }
    } else
    {
      ...
      pprintf("After DEV_TransferPacket() - 0x%8X\n", lChannelError);
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_ERROR,
        	pprintf("Error sending warm start parameter to hardware! (lSendError=0x%08X,ulSta=0x%08X)",
                    lChannelError,
      ...
      } else
      {
        ...
        /*--------------------------------------------*/
    	  ...
    	  lRet = DEV_DoChannelInit( ptChannelInst, CIFX_TO_FIRMWARE_START);
    	  pprintf(" channel init ret val %d", lRet);
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_INFO,
        	  pprintf("Successfully sent warm start parameters to Channel #%d!",
                      ulBlockID);
          }
        } else
        {
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
//        	  pprintf("Hardware not Ready/Running after channel warm start!");
          }
        }
      }
    }
...
//  pprintf("\n channel start address %08X", ulDPMChannelStartAddress);
...
//        pprintf("\n create channel");
      ...
      case RCX_CHANNEL_TYPE_HANDSHAKE:
        ...
        ptDevInstance->pbHandshakeBlock = ptDevInstance->pbDPM + ulDPMChannelStartAddress;
       // pprintf("\n Handshake block start addr %08X", (ptDevInstance->pbHandshakeBlock - ptDevInstance->pbDPM));
        break;
...
//          USER_Trace(ptDevInstance,
//                     TRACE_LEVEL_ERROR,
        	pprintf("Channel (%u) size exceeds the maximum DPM size!",
                     ulChannelID);
        }
     ...
     // pprintf("\n handshake width %02X ",  ptChannelInst->bHandshakeWidth);

      DEV_ReadHostFlags(ptChannelInst, 1);
      DEV_ReadHandshakeFlags(ptChannelInst, 0, 0);

//      pprintf("\n calling readchannel layout %08X", ptChannelInst->)
      /* Read channel layout */
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_ERROR,
        	  pprintf("Channel (%u) does not meet minimum requirement and is ignored!",
                      ulBlockID);
          }
        } else
        {
...
//                USER_Trace(ptDevInstance,
//                          TRACE_LEVEL_WARNING,
            	  pprintf("Error channel not READY, channel = %d\r\n)", ulChannelID);
              }
            } else
            {
        ...
        if( DEV_WaitForReady_Poll(ptChannelInst, 20))
        {
          ...
          memset(&tFWIdentifyReq, 0, sizeof(tFWIdentifyReq));
          memset(&tRecvPkt, 0, sizeof(tRecvPkt));
...
//              USER_Trace(ptDevInstance,
//                        TRACE_LEVEL_WARNING,
            	pprintf("Error querying firmware information! (lRet=0x%08X, ulSta=0x%08X)",
                        lTempError,
                        LE32_TO_HOST(tRecvPkt.tHeader.ulState));
            }
...
//            USER_Trace(ptDevInstance,
//                      TRACE_LEVEL_INFO,
        	  pprintf("Device successfully created for channel = %d\r\n", ulChannelID);
          }
        }

        ++ulChannelID;
        ++ptDevInstance->ulCommChannelCount;
  //      pprintf("\n mem realloc %s  ptDevInstance->pptCommChannels = %08X", __FUNCTION__, ptDevInstance->pptCommChannels);
        ptDevInstance->pptCommChannels = (CHANNELINSTANCE**)OS_Memrealloc(ptDevInstance->pptCommChannels, ptDevInstance->ulCommChannelCount * sizeof(*ptDevInstance->pptCommChannels));
        ptDevInstance->pptCommChannels[ptDevInstance->ulCommChannelCount - 1] = ptChannelInst;
      } else //coverity CID:10952
      {
        /* We have not created a channel, delete the previous allocated channel instance */
		    delete[]  ptChannelInst;//coverity CID: 11203
      }
    }
 ...
 // pprintf("\n %s %d %d",__FUNCTION__, indx++, lRet);
  if( (g_ulTraceLevel & TRACE_LEVEL_INFO) &&
      (0 == ptDevInstance->ulCommChannelCount) )
  {
//    USER_Trace(ptDevInstance,
//               TRACE_LEVEL_INFO,
//	  pprintf("NO CHANNEL INFORMATION FOUND, No devices created!");
  }
...
static CIFX_TOOLKIT_DEVICETYPE_E cifXDetectDeviceType(PDEVICEINSTANCE ptDevInstance)
{
  ...
  {
    char szCookie[5] = {0,0,0,0,0};
    

    pprintf(" %08X", ptDevInstance->pbDPM);
    while((OS_Strcmp( szCookie, CIFX_DPMSIGNATURE_FW_STR)) != 0)
    {
    	//pprintf("\n in loop");
    	HWIF_READN(ptDevInstance, szCookie, ptDevInstance->pbDPM, 4);
    	...
    	RTOS::DelayMs(500);
    	if(retry++ > 10)
    		return eCIFX_DEVICE_AUTODETECT_ERROR;
    }
...
//        USER_Trace(ptDevInstance,
//                   TRACE_LEVEL_ERROR,
//    	  pprintf("DPM device without bootloader detected, try to handle it as a RAM based device.");
      }
...
//          USER_Trace(ptDevInstance,
//                    TRACE_LEVEL_ERROR,
//        	pprintf("Driver is unable to start a RAM based device with a DPM < 64kB.");
        }
      } else
      {
...
//          USER_Trace(ptDevInstance,
//                     TRACE_LEVEL_INFO,
//        	pprintf("RAM based device detected (No FW / Bootloader active)! Card will be reset and all files downloaded!");
        }
        eRet = eCIFX_DEVICE_RAM_BASED;
      }
    }
...
static int32_t cifXStartDevice(PDEVICEINSTANCE ptDevInstance)
{
  ...
  if(eCIFX_DEVICE_AUTODETECT == ptDevInstance->eDeviceType)
  {
    ...
    if(ptDevInstance->eDeviceType != eCIFX_DEVICE_FLASH_BASED)
    	return 1;
...
//          USER_Trace(ptDevInstance,
//                     TRACE_LEVEL_DEBUG,
//        	pprintf("Device Type autodetection: RAM Based Device found!");
        }
        break;
...
//          USER_Trace(ptDevInstance,
//                     TRACE_LEVEL_DEBUG,
//        	pprintf("Device Type autodetection: Flash Based Device found!");
        }
        break;
...
//          USER_Trace(ptDevInstance,
//                     TRACE_LEVEL_ERROR,
        	pprintf("Device Type autodetection FAILED. eDeviceType=%d",
                     ptDevInstance->eDeviceType);
        }
        break;
    }
  }
  pprintf("\n %s-%d ", __FUNCTION__, dummy++);

  switch(ptDevInstance->eDeviceType)
  {
    case eCIFX_DEVICE_RAM_BASED:
    	CRASH("Ram Based Device is not expected");
      /*-----------------------------------*/
    ...
    case eCIFX_DEVICE_FLASH_BASED:
          ...
          (CIFX_NO_ERROR == (lRet = cifXCreateSystemDevice( ptDevInstance)))  )
      {
        ...
        /* Check if we have a BASE OS system to download and to start*/
        lTempResult = CIFX_NO_ERROR;//cifXHandleFlashBaseOSModule( ptDevInstance);
        pprintf("\n skipping checking for download on flash based module");
        if( CIFX_NO_ERROR == lTempResult)
        {
        	int32_t temp;
          /* Check if we have a BASE OS module running */
        	temp = cifXIsBaseOSModule(ptDevInstance);
        	if(temp == CIFX_NO_ERROR){
//        		pprintf("\n Loadable module supported");
        	}
        	else{
//        		pprintf("\n Loadable module not supported");
        	}

//        	pprintf("\n skipping firmware file download on flash based module");
          /* Download firmware / module files */
          //cifXDownloadFWFiles(ptDevInstance, &tDevChannelCfg);

//          pprintf("\n skipping configuration file download on flash based module");
          /* Download configuration files */
          //cifXDownloadCNFFiles(ptDevInstance, &tDevChannelCfg);

//          pprintf("\n confirm that we need to start the flash firmware");
          //return 0;
          //confirm that we need it or not
          /* Start firmware / module files if necessary */
          //cifXStartFlashFirmware(ptDevInstance, &tDevChannelCfg);
          lTempResult = 0;
        }
...
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
//    	  pprintf("Unable to access the hardware while device type is set to eCIFX_DEVICE_DONT_TOUCH. Aborting device handling!");
      }
      break;
    
    case eCIFX_DEVICE_AUTODETECT_ERROR:
    	CRASH("eCIFX_DEVICE_AUTODETECT_ERROR");
      if(g_ulTraceLevel & TRACE_LEVEL_ERROR)
      {
//        USER_Trace(ptDevInstance,
//                  TRACE_LEVEL_ERROR,
//    	  pprintf("Unable to automatically detect device type (RAM or FLASH based device). Aborting device handling!");
      }
      break;
...
//    	pprintf("\n slot number matched");
  ...
  if(CIFX_NO_ERROR == lRet)
  {
    ...
    if(CIFX_NO_ERROR == lRet)
    {
    	//pprintf("\n skipping cifXHandleWarmstartParameter");
...
static int32_t cifXStopDevice(PDEVICEINSTANCE ptDevInstance)
{
  ...
  /*-------------------------------------------------*/
  if(ptDevInstance->pptCommChannels)
  ...
  } else
  {
    /* More device existing, shrink memory */
	  pprintf("\n mem realloc %s  g_pptDevices = %08X", __FUNCTION__, g_pptDevices);
    g_pptDevices = (DEVICEINSTANCEtag**)OS_Memrealloc(g_pptDevices, g_ulDeviceCount * sizeof(*g_pptDevices));
  }
  pprintf("\n %s-%d ", __FUNCTION__, dummy++);
  return CIFX_NO_ERROR;
}
...
int32_t cifXTKitAddDevice(PDEVICEINSTANCE ptDevInstance)
{
  ...
  pprintf("\n returning from %s-%d ", __FUNCTION__, lRet);
  //under testing
...
*   \return CIFX_NO_ERROR on success                                         */
/*****************************************************************************/
int32_t cifXHandleWarmstartParameter(PDEVICEINSTANCE ptDevInstance, CIFX_PACKET * tPacket)
{
