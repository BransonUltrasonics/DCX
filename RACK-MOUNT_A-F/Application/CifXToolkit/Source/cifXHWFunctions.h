 ...
 **************************************************************************************
   ...
   Filename:
    $Id:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/Application/CifXToolkit/Source/cifXHWFunctions.h_v   1.0   27 Jul 2015 08:55:32   sesharma  $
   Last Modification:
    $Author:   sesharma  $
...
//  #define HWIF_WRITEN(ptDev, Dst, Src, Len) OS_Memcpy(Dst, Src, Len)
  //WriteDPRam(uint32_t(Dst), Len, (void *)Src)
  //OS_Memcpy(Dst, Src, Len)
...
void      USER_Trace                    (PDEVICEINSTANCE ptDevInstance, uint32_t ulTraceLevel,
  ...
  #define HWIF_READN(ptDev, Dst, Src, Len) ((PDEVICEINSTANCE)ptDev)->pfnHwIfRead(ptDev, (uint32_t)Src, Dst, Len)
   ...
   do{                           \
   uint8_t bData = Src;       \
   HwIfWrite8(ptDev , (void *)&Dst, &bData);  \
   }while(0);

   #define HWIF_WRITE16(ptDev, Dst,  Src)    \
   do{                            \
   uint16_t bData = Src;          \
   HwIfWrite16(ptDev , (void *)&Dst, &bData); \
   }while(0);

   #define HWIF_WRITE32(ptDev, Dst, Src)   \
   do{                      \
   uint32_t bData = Src;       \
   HwIfWrite32(ptDev , (void *)&Dst, &bData); \
   }while(0);
  #define HWIF_WRITEN(ptDev, Dst, Src, Len) ((PDEVICEINSTANCE)ptDev)->pfnHwIfWrite(ptDev, (uint32_t)Dst, Src, Len)
