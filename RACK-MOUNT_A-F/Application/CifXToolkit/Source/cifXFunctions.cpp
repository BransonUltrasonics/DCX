 ...
 **************************************************************************************
   ...
   Filename:
    $Workfile:   cifXFunctions.cpp  $
   Last Modification:
    $Author:   sesharma  $
    $Modtime:   24 Jul 2015 12:19:20  $
    $Revision:   1.0  $
...
//  pprintf("\n device count = %d", g_ulDeviceCount);
  for(ulIdx = 0; ulIdx < g_ulDeviceCount; ++ulIdx)
  {
//	  printf(" \n szBoard %s g_pptDevices[ulIdx]->szName %s CommChannelCount %d", szBoard, g_pptDevices[ulIdx]->szName, g_pptDevices[ulIdx]->ulCommChannelCount);
...
//	  printf("\n %s %d", __FUNCTION__, indx++);
 ...
 //     pprintf("\n %s %08X ", __FUNCTION__, &ptIOArea->pbDPMAreaStart[ulOffset]);
      /* Lock flag access */
      OS_EnterLock(ptChannel->pvLock);
 ...
 // printf("\n %s %d ret %d", __FUNCTION__, indx++,lRet);
  return lRet;
}
                                      ...
                                      PFN_RECV_PKT_CALLBACK  pfnRecvPktCallback, void* pvUser)
{
  ...
  memset(&tDirListCnf, 0, sizeof(tDirListCnf));
  ...
  switch (ulNotification)
  {
    case CIFX_NOTIFY_RX_MBX_FULL:
      ...
      {        
        ...
        if(DEV_WaitForBitState((PCHANNELINSTANCE)hChannel,
    ...
    case CIFX_NOTIFY_SYNC:
      ...
      } else
      {
        ...
        if(DEV_WaitForSyncState((PCHANNELINSTANCE)hChannel,
                                bState,
                                0))
        {
          pfnCallback(ulNotification, 0, NULL, pvUser);
  ...
  {  
    switch (ulCmd)
    {
      case CIFX_SYNC_SIGNAL_CMD:
        ...
        } else if(!DEV_WaitForSyncState((PCHANNELINSTANCE)hChannel, RCX_FLAGS_EQUAL, ulTimeout))
        {
      ...
      case CIFX_SYNC_ACKNOWLEDGE_CMD:
        ...
        } else if(!DEV_WaitForSyncState((PCHANNELINSTANCE)hChannel, RCX_FLAGS_NOT_EQUAL, ulTimeout))
        {
          /* Sync cannot be signalled as bits are in wrong state */
          lRet = CIFX_DEV_SYNC_STATE_TIMEOUT;
        } else
      ...
      case CIFX_SYNC_WAIT_CMD:
        {
          ...
          } else
          {
            ...
            /* Wait for sync */
            if(!DEV_WaitForSyncState((PCHANNELINSTANCE)hChannel, bState, ulTimeout))
            {
              /* Sync timeout */
              lRet = CIFX_DEV_SYNC_STATE_TIMEOUT;
            } else
...
uint8_t HwIfRead8(void * ptDev,  void * Src)
{
	uint8_t data;
	ptDev = 0;
	OS_Memcpy(&data, Src, 1);
	return data;
}


uint16_t HwIfRead16(void * ptDev, void *Src)
{
	uint16_t data;
	ptDev = 0;
	OS_Memcpy(&data, Src, 2);
	return data;
}

uint32_t HwIfRead32(void * ptDev, void *Src)
{
	uint32_t data;
	ptDev = 0;
	OS_Memcpy(&data, Src, 4);
	return data;
}

void HwIfWrite8(void * ptDev,  void * Dst, void * Src)
{
	ptDev = 0;
	OS_Memcpy(Dst, Src, 1);
}

void HwIfWrite16(void * ptDev, void * Dst, void * Src)
{
	ptDev = 0;
	OS_Memcpy(Dst, Src, 2);
}
void HwIfWrite32(void * ptDev, void * Dst, void * Src)
{
	ptDev = 0;
	OS_Memcpy(Dst, Src, 4);
}
