// $Header:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/CyGOS/Arch/MCF/MCF5445x/CPU.cpp_v   1.1   24 Jul 2015 08:58:18   sesharma  $
//
 ...
 * @param wait Wait states. If -1 then no wait states are used and
 *        Auto-Ack is turned off.
...
void CPU::SetupDefaultHandler(ExceptionHandler handler, CpuInterrupt vector,
		int irqlevel, int prio, int stackSize)
{
	//Disable compiler warning
	prio = 0;
	...
	if (irqlevel == 7)
		ASSERT(vector == intEPFR7);
	Exception::SetDefault(IntVector(vector), handler, stackSize);
}
/**
