// $Header:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/CyGOS/Arch/MCF/MCF5445x/DSPI.cpp_v   1.1   24 Jul 2015 08:58:18   sesharma  $
//
...
void DSPI::IntHandler()
{
	...
	else
	{
		...
		for (int i = 0; i < length; i++)
		{
			...
			if (i == (transferLength - 1))
				cont = 0;
			int tx = 0 | MCF_DSPI_DSPI_PUSHR_CTAS(transferCtar)
					| cont | transferCS // CS to assert
					| MCF_DSPI_DSPI_PUSHR_TXDATA(transferData[i]);
			reg->PUSHR = tx;
		}
