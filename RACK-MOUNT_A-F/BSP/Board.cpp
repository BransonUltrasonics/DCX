// $Header:   D:/databases/VMdb/archives/DCX/RackMount/A&F/WC/BSP/Board.cpp_v   1.0   24 Jul 2015 09:08:08   sesharma  $
//
...
const int Board::spiNum = 1;
const int Board::i2cNum = 1;
const int Board::profileTimerNum = 0;
SPI * Board::spi[Board::spiNum];
I2C * Board::i2c[Board::i2cNum];
...
void Board::RemoveTimer(int instance)
{
	PIT::Remove(instance);
}
...
I2C * Board::GetI2C(int instance, int level, int priority, int clk)
{
	...
	if (!i2c[instance])
		...
		i2c[instance] = new I2C(instance, 0, level, priority, clk);
	return i2c[instance];
}
/**
 ...
 */
void Board::InitProfilerTimer()
{
	CriticalSection s;
	ASSERT((profileTimerNum >= 0 && profileTimerNum <= 3));
	ASSERT((dmaTimers & (1 << profileTimerNum)) == 0);
	MCF_DTIM_DTMR(profileTimerNum) = MCF_DTIM_DTMR_CLK(1)
			| MCF_DTIM_DTMR_PS(__SYSTEM_CLOCK - 1);
	MCF_DTIM_DTXMR(profileTimerNum) = 0;
	MCF_DTIM_DTCN(profileTimerNum) = 0;
	MCF_DTIM_DTMR(profileTimerNum) |= MCF_DTIM_DTMR_RST;
	dmaTimers |= 1 << profileTimerNum;
}

int Board::GetProfilerTimerCounter()
{
	return MCF_DTIM_DTCN(profileTimerNum);
}
