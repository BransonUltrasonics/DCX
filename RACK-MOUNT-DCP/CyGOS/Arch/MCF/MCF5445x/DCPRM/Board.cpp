// $Header:   D:/databases/VMdb/archives/DCX/RackMount/DCP/CyGOS/Arch/MCF/MCF5445x/DCPRM/Board.cpp_v   1.1   28 Jan 2015 07:28:42   sesharma  $
//
...
void InitExternalWatchDogTrigger()
{
  MCF_PAD_PAR_TIMER |= (MCF_PAD_PAR_TIMER_PAR_T1IN(MCF_PAD_PAR_TIMER_PAR_T0IN_T0OUT));
...
{
	new PIT(instance, (usInterval * __SYSTEM_CLOCK), 6, 7,
			callback, 256);
	printf("T %dus",(int)usInterval);
}
...
{
	//This board with MCF54451 chip only has
...
//	DisableInterrupts();
   ...
   MCF_DTIM_DTMR(instance) |= MCF_DTIM_DTMR_CLK(1);//120/1
   MCF_DTIM_DTMR(instance) |= MCF_DTIM_DTMR_PS(119) ;
