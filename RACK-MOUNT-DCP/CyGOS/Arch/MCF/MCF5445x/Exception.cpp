// $Header:   D:/databases/VMdb/archives/DCX/RackMount/DCP/CyGOS/Arch/MCF/MCF5445x/Exception.cpp_v   1.1   28 Jan 2015 05:45:04   sesharma  $
//
...
void Exception::FirstHandler(void * frame)
{
	...
	if (RTOS::IntNesting() != 0)
	{
		...
		pprintf("\n\n\n");
		CyGOS::WriteSystemEvent(FAULT_ERROR, (unsigned short)oldLevel , MCF5XXX_RD_SF_VECTOR(frame), 1, true);
 ...
 * @param handler Function to handle the interrupt.
 */
void Exception::SetDefault(int vector, ExceptionHandler, int stackSize)
{
	ASSERT(ExceptionHandlerArray[vector] != DefaultHandler);
	ExceptionHandlerArray[vector] = DefaultHandler;
	stackSize = 0;//Avoid compiler warning (unused variable);
	/*if (stackSize > DefaultStackSize)
	{
		exceptionHandlerStack[vector] = new uint8[stackSize];
