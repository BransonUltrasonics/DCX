// $Header:   D:/databases/VMdb/archives/DCX/RackMount/DCP/CyGOS/RTOS/RTOS.cpp_v   1.1   28 Jan 2015 05:52:52   sesharma  $
//
...
void RTOS::PrintStatus(PrintStatusMode mode)
{
	...
	for (int prio = 0; prio < prioTotal; prio++)
	{
		...
		if (mode == PrintTasksWithStack)
			printf("Stack: %08X (%6ld) ", (unsigned int) task->stack, (task->stack
					- task->stackbegin) * 4);
		if (task->waitMutex)
		{
