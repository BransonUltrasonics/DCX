/*
$Header:   D:/databases/VMdb/archives/DCX/DCP/DCP Application/DCX2010/StateMachineDrv.cpp_v   1.8.1.11.1.2.1.2.1.2   Jul 07 2017 12:51:44   hasanchez  $
*/
/****************************************************************************/
/*                                                                          */
/*                                                                          */
 ...
 * 
 *    Rev 1.8.1.11.1.2.1.2.1.2   Jul 07 2017 12:51:44   hasanchez
 * Updated to make the code more readable
 * 
 *    Rev 1.8.1.11.1.2.1.2.1.1   Apr 05 2017 13:51:46   hasanchez
 * Updated the variable name WeldErrorFinish to WeldErrorFinishFlag
 * 
 *    Rev 1.8.1.11.1.2.1.2.1.0   Mar 29 2017 09:29:28   hasanchez
 * Updated location of CycleStarted flag.
 * 
 *    Rev 1.8.1.11.1.2.1.2   Feb 09 2017 15:29:22   hasanchez
 * Added functionality related tot he Manager Basic.
 * 
 *    Rev 1.8.1.11.1.2.1.1   29 Apr 2016 09:41:28   hasanchez
 * Coding standard applied
 * 
 *    Rev 1.8.1.11.1.2.1.0   17 Mar 2016 16:23:46   hasanchez
 * Added the changes to the function Adjust Stack
 * 
 *    Rev 1.8.1.11.1.3   17 Mar 2016 16:07:24   hasanchez
 * Added the changes to include the function Adjust amplitude.
 * 
 *    Rev 1.8.1.11.1.2   02 Sep 2014 07:01:50   amaurya
 * Added RampRequired SubState.
 * 
 *    Rev 1.8.1.11.1.1   13 Feb 2013 09:21:32   amaurya
...
/****************************************************************************/
{
	...
	else if((PowerUpTestStartedFlag == FALSE) && (RunningPreset.SWDip.Flags.PowerUpOptions == POWERUPTEST))
	{
		NoPowerUpSeekScan = FALSE;
		if(PowerUpDelay <= TotalPowerUpDelay)  //PowerUp Delay
		{
		...
		{
			//CurrentPreset.HDParam.InitPara[0]=
			TempMode=RunningPreset.HDParam.InitPara[HD_MODE];
			RunningPreset.HDParam.InitPara[HD_MODE]=AMPLITUDE_LOOP_ON;
			...
			PowerUpDelay = 0;
			PowerUpCompleted = TRUE;
			PowerUpPWMAmpFlag = TRUE;
			WebSite::mfTestFlag = TRUE;
			PowerUpTestStartedFlag = TRUE;
...
//	AutoSonicsEnable = FALSE;
	...
	if (WRSendFlag==TRUE && CycleStarted == TRUE ) //if last state flag was WeldComplete
...
/****************************************************************************/
{
	...
	if(EnableWebIO)
		NoInput = TRUE;
	else if(!(UserIO::CheckForExternalReset() || WebSite::CheckForWebReset() || UserIO::CheckForExternalStart() || UserIO::CheckForExternalTest() || UserIO::CheckForPWMAmpAdjust_Input()
...
//		pprintf("\n cycle stopped 1");
	...
	if (WRSendFlag==TRUE && CycleStarted == TRUE)
		BroadcastWR::SendWR();    // This to send Weld Result via ethernet
	CycleStarted = FALSE;
}
...
/****************************************************************************/
{
	//CycleStarted = FALSE;  deleted 01/27/17  HSAN
	WeldErrorFinishFlag = FALSE;
   SeekEnable = FALSE;
   TimedSeekEnabled = FALSE;
   StartInputEnable = FALSE;
...
SSTATES WaitForPowerUp [] = {{&ResetEnable,      SKIP,   NORMAL, NONE, 	 RESET_ERROR_IDX,    102},
                             ...
                             {&PowerUpPWMAmpFlag,	SKIP,   NORMAL, NONE,    TEST_START_IDX,    106},
...
SSTATES WaitForInput[]  = {  {&ResetEnable,      SKIP,   NORMAL, NONE, 	 RESET_ERROR_IDX,   102},
							        ...
							        {&PWMAmpAdjustFlag,		SKIP,   NORMAL, NONE,  	 TEST_START_IDX,    106},
...
SSTATES CheckTestStart[] = {{&ResetEnable,       SKIP,   NORMAL, NONE, 	 RESET_ERROR_IDX,   102},
									 ...
									 {&PWMAmpAdjustFlag,   SKIP,   NORMAL, NONE,    SEEK_START_IDX,    130},
...
SSTATES CheckSeekStop[]=  { {&ResetEnable,       SKIP,   NORMAL,  NONE, 	 RESET_ERROR_IDX,   102},
						          ...
						          {&PWMAmpAdjustFlag, 	   SKIP,   NORMAL, NONE,    SEEK_WELD_IDX,     145},
...
SSTATES CheckWeldStop[]=  {  {&ResetEnable,      SKIP,   NORMAL, NONE, 	 RESET_ERROR_IDX,   102},
									  {&OverLoadAlarm,		SKIP,   NORMAL, NONE, 	 WELD_ERROR_IDX,    102},
									  {&PWMAdjustONFlag,	SKIP,   NORMAL, NONE,    WAITNOINPUT_IDX,       166},
...
SSTATES CheckWeldError[]= {  {&ResetEnable,      SKIP,   NORMAL, NONE, 	 RESET_ERROR_IDX,   102},
									  {&WeldErrorFinishFlag,  SKIP,   NORMAL, NONE,    WAIT_STOP_IDX,     169},
						           {&TimerFlag,        END,    END,    NONE,    NONE,              170}};

SSTATES CheckManFWeldError[]= {
		                     {&ResetEnable,        SKIP,   NORMAL, NONE, 	 RESET_ERROR_IDX,   102},
								   {&WeldErrorFinishFlag,	   JUMP,   NORMAL, NONE,    NONE,     		     157},
						         {&TimerFlag,          END,    END,    NONE,    NONE,              170}};


SSTATES CheckResetError[]={  {&ResetComplete,	   SKIP,   NORMAL, NONE,    WAITING_IDX,       172},
...
SSTATES CheckPreWait[]= {    {&ResetEnable,      SKIP,   NORMAL, NONE, 	 RESET_ERROR_IDX,   102},
