/*
$Header:   D:/databases/VMdb/archives/DCX/DCP/DCP Application/DCX2010/UserIO.cpp_v   1.21.2.4.1.2   Aug 14 2018 12:11:28   EGUAJARDO  $
*/
/****************************************************************************/
/*                                                                          */
/*                                                                          */
 ...
 * 
 *    Rev 1.21.2.4.1.2   Aug 14 2018 12:11:28   EGUAJARDO
 * Removed DAC factors in SetOutputPower function.
 * 
 *    Rev 1.21.2.4.1.1   Feb 09 2017 16:09:42   hasanchez
 * added the option to create an analog output on 4.5kw range
 * 
 *    Rev 1.21.2.4.1.0   17 Mar 2016 16:23:48   hasanchez
 * Added the changes to the function Adjust Stack
 * 
 *    Rev 1.21.2.5   17 Mar 2016 16:07:26   hasanchez
 * Added the changes to include the function Adjust amplitude.
 * 
 *    Rev 1.21.2.4   10 Mar 2015 07:53:00   amaurya
 * Add feature “energy in range 250J" on the analog inputs.
 * 
 *    Rev 1.21.2.3   02 Sep 2014 07:02:18   amaurya
 * Added Amplitude Ramping.
 * 
 *    Rev 1.21.2.2   02 Jul 2014 09:40:16   amaurya
 * Removed Amplitude Decay function.
 * 
 *    Rev 1.21.2.1   13 Feb 2013 09:22:08   amaurya
...
static BOOLEAN ExtReset,ExtAmpStep,ExtSeek,RecallPreset1, RecallPreset2,RecallPreset4,RecallPreset8,
			...
			StartHornScan, ExtTest,	HornDownMode,MemClear, CableDetect, PWMAmpAdjust_Input;
...
DIGITALIOMAP InPtr[NUM_FUNC_IN + 1] = {
	...
	{&PWMAmpAdjust_Input,  HIGH, 	(ALL)},
	{&PWMAmpAdjust_Input, 	LOW, 	(ALL)},
...
powerOut10,powerOut11,powerOut12,powerOut13,amplitudeOut, freqOut, memOut,currentOut,EnergyOut;
...
/******************************************************************************/
{
	...
	dac = new TLV5604(qspi, 0x02, REFVOLTAGE_DAC, REFVOLTAGE_DAC);
	adc = new MCP3208(qspi, 0x04, REFVOLTAGE_ADC);
...
//	/**  Now we need to take bits and convert to BOOLEAN variables for system use  **///
	...
	for(Loop=1; Loop< NUM_AIN; Loop++)
	{
		if((AnalogInValue[Loop] != NULL) && (AnalogInUseStatus[Loop] == ACTIVATED))
		{
			...
			*AnalogInValue[Loop] =	adc->ReadInput(ch - 1);
		}
			//ch |= (1 << InChannles[Loop]);
	}
}
...
/******************************************************************************/
{
	...
	UserIO::AssignDigitalInputFunction(DIGIN2 , PWMADJUST_ACTIVEHIGH,ACTIVATED);//Stack Adjustment active high
...
BOOLEAN UserIO::CheckForPWMAmpAdjust_Input(void)
{
	return CheckSignalActive(PWMAmpAdjust_Input);
}
...
void UserIO::SetOutputCurrent(SINT32 val)
{
	currentOut = val;
}
