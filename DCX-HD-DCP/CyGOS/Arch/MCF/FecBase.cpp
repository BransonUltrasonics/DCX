// $Header:   D:/databases/VMdb/archives/DCX/DCP/DCP Application/CyGOS/Arch/MCF/FecBase.cpp_v   1.6.2.0   19 Sep 2014 07:41:48   amaurya  $
//
...
FecBase::FecBase(int baseAddress, uint8 * addr) :
	rxQueue(nRxBD + 10, "RxBD"), txQueue(nTxBD, "TxBD")
{
...
void FecBase::RxHandler()
{
	for (;;)
	{
		...
		if (keep)
		{
			bool received = rxQueue.Write(0, pRxBD, 0);
			if (!received)
			{
				ReleaseBD(pRxBD);
			}
...
{
	return mii->ReadRegister(0x01) & 0x0004;
}
