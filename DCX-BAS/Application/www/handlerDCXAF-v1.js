...
$('#footer').ready(function () {
	UseID('footer').innerHTML =  "<p> &copy;2013 Branson, All Rights Reserved<br />"+"<img src=\"Emerson-IA.jpg\" alt=\"Emersin Industrial Automation\" /></p>";
});
...
function IfValid(ElementName,ElementID){
	...
	if((UseID(ElementID).value.length != 0) && (temp[0] != '+') && (!isNaN(temp))){
		temp = UseID(ElementID + "MaxMin").value.split(',');
		if(UseID(ElementID).value < parseFloat(temp[0]) || UseID(ElementID).value > parseFloat(temp[1]))
			returnstr = "* " + ElementName + " should be between " + temp[0] + " to " + temp[1] + "\n";
	}
	else
