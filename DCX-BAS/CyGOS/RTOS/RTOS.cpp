// $Header:   D:/databases/VMdb/archives/DCX/Weld Controller/WC/CyGOS/RTOS/RTOS.cpp_v   1.10   08 Oct 2013 05:48:30   rjamloki  $
//
 ...
 */
//int Dtim_counter;
//TaskInformation taskInfo[64]; //Maximum 64 tasks in CyGOS.Index is representing the priority of task
//TaskInformation interruptInfo[255];
//int priorityInSchedular;
IdleTask::IdleTask() :
	Task("Idle", 1024)
{
	ctr = 0;
...
//	memset(interruptInfo,0,sizeof(interruptInfo));
...
//		if (task) // task exists?
//		{
//			if(strlen(task->name) > 19)
//				ASSERT(0);
//			strcpy(taskInfo[i].name , task->name);
//		}
//	}
	if (!running)
	{
...
void RTOS::TickHandler()
{
	...
	for (prio = 0; prio < prioTotal; prio++)
	{
		...
		if (task)
		{
			...
			if (task->stackbegin[0] != (long)0xDEADBABE)
...
void RTOS::PrintStatus(PrintStatusMode mode)
{
	...
	for (int prio = 0; prio < prioTotal; prio++)
	{
		...
		printf("%2d %20s %-10s ", prio, task->name,
				...
				task->state == Running ? "Running" : "Unknown");
		if (mode == PrintTasksWithStack)
			printf("Stack: %08X (%6ld) ", (int) task->stack,
					(task->stack - task->stackbegin) * 4);
		if (task->waitMutex)
		{
