...
byte * FileDecode::UnCompress(byte * sourceBytes,
		int & resultLength, int CompressedLen)
{
	Index = 0;
	DIndex = 0;
	...
	Decode(CompressedLen);
	return _pDstBytes;
}

void FileDecode::Decode(int SrcByteLen)
{
	int i, j, k, r, c;
	...
	for (count = 0; count < textsize;)
	{
		...
		{
			if((DIndex) >=  textsize){
				pprintf("\n exceeded dst buff in func %s", __FUNCTION__);
			}
			_pDstBytes[DIndex++] = c;
			text_buf[r++] = c;
			r &= (N - 1);
		...
		else
		{
			...
			{
				if(((i + k) & (N - 1)) > (N + F - 1)){
					pprintf("\n Exceeding buff access at line %d", __LINE__);
				}
				c = text_buf[(i + k) & (N - 1)];

				if((DIndex) >=  textsize){
					pprintf("\n exceeded dst buff in func %s line %d", __FUNCTION__, __LINE__);
				}
				_pDstBytes[DIndex++] = c;
				//pprintf(" else c=%x", c);
				text_buf[r++] = c;
...
int FileDecode::GetBit(void) /* Get one bit (0 or 1) */
{
	...
	{
		if((Index) >= (CompLen))
			pprintf("\n Out of index access %d in %s ", Index , __FUNCTION__);
...
int FileDecode::DecodeChar(void)
{
	...
	sym = BinarySearchSym((unsigned int) (((value - low + 1) * sym_cum[0] - 1)
			/ range));
	if(((sym - 1) >= (N_CHAR + 1)) || (sym > (N_CHAR + 1)))
		pprintf("\n Excedded in sym buffer at index", sym);
 ...
 i such that sym_cum[i - 1] > x >= sym_cum[i] otherwise */
{
	...
	while (i < j)
	{
		k = (i + j) / 2;
		if(k >= (N_CHAR + 1)){
			pprintf("\n excedded in func %s line %d", __FUNCTION__, __LINE__);
		}
		if (sym_cum[k] > x)
			i = k + 1;
		else
...
void FileDecode::UpdateModel(int sym)
{
	...
	i = sym;
	if((i >= (N_CHAR + 1)) || (i == 0)){
			pprintf("\n Excedded sym buff at index %d line %d sym %d", i, __LINE__, sym);
	}
	for (; sym_freq[i] == sym_freq[i - 1];){
		if(i <= 0)
			pprintf("\n Excedded sym buff at index %d line %d", i, __LINE__);
		i--;
	}
	...
	if(i >= (N_CHAR + 1)){
		pprintf("\n Excedded sym buff at index %d", i);
	}
	sym_freq[i]++;
	while (--i >= 0)
		sym_cum[i]++;
...
int FileDecode::DecodePosition(void)
{
	...
	if(position >= (N + 1)){
		pprintf("\n excedded buff access at line %d", __LINE__);
	}
