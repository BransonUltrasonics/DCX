// $Header:   D:/databases/VMdb/archives/DCX/DCP/DCP Application/CyGOS/Arch/MCF/MCF5445x/DSPI.h_v   1.2   05 Mar 2013 06:38:00   amaurya  $
//
...
public:
	 ...
	 */
	void EnableSPI(bool phase, int bits, int clock, bool polarity, bool lsb, uint32 DelayParams);
	...
	void LongWordTransfer(int length, uint8 * data, int maxLength,
			int chipSelect);
	uint32 SetDelayParams(int pcssck, int pasc, int pdt, int cssck,
	         int asc, int dt);
	/**
	 * 	Variables
	 */
