// $Header:   D:/databases/VMdb/archives/DCX/DCP/DCP Application/CyGOS/Arch/MCF/MCF5445x/DSPI.cpp_v   1.5   05 Mar 2013 06:37:42   amaurya  $
//
 ...
 */
void DSPI::EnableSPI(bool phase, int bits, int clock, bool polarity, bool lsb, uint32 DelayParams)
{
...
//	}
	...
	if((clock != Speed) || (polarity != Polarity) || (phase != Phase) || (DelayParameters != DelayParams) || !spiflag)
   {
      baud = GetBaudRate(__SYSTEM_CLOCK * 1000000, clock,
            polarity, phase); // sck frequency
     // pprintf("\n %s baud %08X",__FUNCTION__, (ctar | baud));
   }
...
uint32 DSPI::SetDelayParams(int pcssck, int pasc, int pdt, int cssck,
            int asc, int dt)
{
   uint32 ctar = 0;
   ctar |= (MCF_DSPI_DSPI_CTAR_PCSSCK(pcssck) | MCF_DSPI_DSPI_CTAR_PASC(pasc) |
